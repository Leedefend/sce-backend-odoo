...
因此，当前系统已经具备旧报表主体业务事实承载能力，但还不能宣称“完全等价旧报表”。Batch-L 已修复项目级资金余额在报表行上的覆盖问题；剩余需要复核的是抵扣比例是否应保持往来单位维度还是调整为项目级口径。
...
1. 回查旧过程 `SF` 最终计算层，决定抵扣比例继续按行计算，还是改为项目级比例并在往来单位行重复展示。
2. 复核 Batch-L 新增的“项目级余额”行在原生列表与自定义前端中的显示效果。
3. 重跑模拟生产覆盖矩阵，确认旧报表 27 字段达到“可办理后可查看、可解释、可重建”的状态。
