...
- 已承载并可用于业务查看：`25` 个
- 已承载但需业务口径复核：`1` 个，`SF` 抵扣比例
- 已承载但存在报表行覆盖缺口：`1` 个，`SJKYYE` 实际可用余额
- 未承载字段：`0` 个

因此，当前系统已经具备旧报表主体业务事实承载能力，但还不能宣称“完全等价旧报表”。下一轮必须优先修复项目级资金余额在报表行上的覆盖问题，并复核抵扣比例是否应保持往来单位维度还是调整为项目级口径。
...
| `SF` | 税负/抵扣比例 | `tax_deduction_rate` | 需复核 | 当前按项目+往来单位行计算，运行库只有 2 行同时具备销项税和抵扣税。需确认旧系统是否实际按项目级比例展示。 |
| `SJKYYE` | 实际可用余额 | `actual_available_balance` | 存在覆盖缺口 | 项目级事实已承载，但报表只在已有往来单位行上显示；项目余额事实 755 条，当前报表仅覆盖 512 个项目。 |
...
- 当前报表中有实际可用余额的项目：`512` 个
- 当前报表行级 `actual_available_balance` 不允许直接合计，因为同一项目余额会随往来单位行重复展示

## 5. 当前缺口

### P0：实际可用余额报表行覆盖缺口

`SJKYYE` 是项目级余额。当前投影只把它挂到已有项目+往来单位行上。如果某项目只有资金余额事实、没有合同/收款/付款/发票/自筹等往来单位事实，该项目不会出现在 `sc.ar.ap.project.summary`。

下一轮应把 `project_fund_balance` 纳入报表 key 集合，生成“项目级余额”行或等价的业务行，保证 755 条项目余额事实均可被用户看到。
...
但运行库只有 2 行同时有销项税额与抵扣税额。原因是销项税通常在客户往来单位行，抵扣税通常在供应商往来单位行。下一轮需要回查旧过程最终 SELECT，确认 `SF` 是按项目+往来单位计算，还是按项目级汇总比例展示。

## 6. 下一轮执行建议

1. 修复 `SJKYYE` 覆盖：让项目资金余额事实独立进入 `sc.ar.ap.project.summary` key 集合，并使用清晰的往来单位显示名，避免用户误解为某个供应商/客户余额。
2. 回查旧过程 `SF` 最终计算层，决定抵扣比例继续按行计算，还是改为项目级比例并在往来单位行重复展示。
3. 修复后重跑模拟生产覆盖矩阵，确认旧报表 27 字段达到“可办理后可查看、可解释、可重建”的状态。
