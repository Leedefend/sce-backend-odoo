# Project Write Trial Rollback Target v1

## Target Lock Rule

Rollback targets are locked only by `legacy_project_id`. Do not use project
name, company text, specialty text, `other_system_id`, `other_system_code`, or
Odoo record names as the rollback selector.
...
| locked identities | 30 |
| records found | 30 |
| missing identities | 0 |
| duplicate identities | 0 |

## Locked Rollback Targets
...
Rollback target locking is clear: 30 target records are uniquely identified by
the 30 approved `legacy_project_id` values.
