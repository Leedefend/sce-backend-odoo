...
- 已补第七步：新增 `make verify.business_config.low_code_acceptance`，用业务配置管理员账号打开 `/admin/business-config`，验证默认用户路径不暴露治理/技术话术，并覆盖页面搜索/筛选、运行页面预览、列表与搜索三类 tab、列表与搜索草稿编辑和放弃调整、表单字段点选/拖拽/显示隐藏/顺序调整/新增字段、检查效果、返回配置上下文；验收报告落盘关键截图，并检查浏览器 error/warning 为空。
