...
## 8) v1.0.0 Formal Release Line

The first formal release line uses the standard release format:

- gate baseline: `gate-release-v1.0`
- release candidates: `v1.0.0-rc1`, `v1.0.0-rc2` only when blocker fixes require a new candidate
- formal release: `v1.0.0`

Promotion order:

1. Merge reviewed release-prep work to `main`.
2. Run `make verify.release.v1_0_0.preflight` on the reviewed release commit.
3. Create `gate-release-v1.0` only after gate evidence passes.
4. Run `make verify.release.v1_0_0.product_hardening` and close any blocker.
5. Create `v1.0.0-rc1` only after RC evidence passes.
6. Create `v1.0.0` only after prod-sim acceptance and release checklist signoff.

Production deployment is not implied by creating `v1.0.0`; deployment remains a
