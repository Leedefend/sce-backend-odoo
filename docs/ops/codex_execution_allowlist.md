...
其职责是在 **独立分支（feature/* / feat/* / fix/* / codex/* / experiment/*）** 内，
...
```
^(feat|feature|fix|codex|experiment)\/.+
```
...
* `make pr.push`
  ...
  * 将当前分支依次 push 到 GitHub `origin` 与 CI 镜像 `gitee`，用于 **更新 PR 的代码内容并保证 CI 可检出同一提交**
  * 必须校验：
    ...
    * 非 main / release / prod
    * 工作区干净（包括未跟踪文件）
    * `origin` 与 `gitee` 两个 remote 均已配置
    * 在首次 push 前，`origin` 与 `gitee` 均通过只读可访问性预检；任一失败则零 push 退出

  * 预检成功后才依次写入 `origin`、`gitee`；若 `origin` 成功而 `gitee` 因竞态或网络中断失败，必须以非零状态报告 `partial_sync`、成功/失败远端和 `make pr.push` 恢复命令
...
* Codex 执行任何 **Git 写操作** 前，必须确认：
  ...
  * 当前分支 ∈ {feat/*, feature/*, fix/*, codex/*, experiment/*}
...
在合规分支（`feat/*` `feature/*` `fix/*` `codex/*` `experiment/*`）内，
