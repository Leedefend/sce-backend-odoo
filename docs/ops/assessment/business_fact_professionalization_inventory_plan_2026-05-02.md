...
| WP6 材料进场验收/入库 | `sc.material.acceptance`、`sc.material.acceptance.line`、`sc.material.inbound`、`sc.material.inbound.line` | `models/core/material_acceptance.py`、`views/core/material_acceptance_views.xml` | 从 `sc.material.document(inbound)` 拆出到场验收与库存入库两个专业动作，现有“入库单”入口只承载入库单 |
