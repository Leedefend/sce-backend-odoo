# Phase X Operating Summary Report

- gate_pass_count: 9/9
- error_count: 0
- warning_count: 0

## Snapshot

- capability_count: 42
- domain_count: 6
- isolated_capability_count: 1
- over_authorized_profile_count: 0
- role_sample_count: 8
- runtime_scene_count: 164
- semantic_drift_count: 0
- stress_error_count: 0
- stress_status: pass
- stress_warning_count: 0
- trend_days: 7
- trend_history_day_count: 1
- unclassified_404_count: 0
- unknown_domain_count: 0
- unknown_source_count: 0
- unresolved_intent_count: 0
- week1_error_count: 0
- week2_error_count: 0

## Gates

- button_404_fully_classified: PASS
- capability_explainable: PASS
- role_sample_ready: PASS
- runtime_source_explained: PASS
- scene_fully_assigned: PASS
- semantic_guard_clean: PASS
- stress_no_errors: PASS
- week1_gate_clean: PASS
- week2_gate_clean: PASS
