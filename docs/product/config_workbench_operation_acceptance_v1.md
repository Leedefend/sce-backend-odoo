...
每项 0 到 2 分，满分 20 分。正式交付线为总分不低于 18 分，且任一单项不得为 0 分。
...
| 任务效率 | 完成主路径需要明显绕路 | 路径可完成但步骤偏多 | 关键任务 3 次主要点击内进入目标配置面 |
...
| 配置列表与搜索 | 进入后能看到列表列、搜索条件、默认分组三个常见配置类型 | 列表搜索面板截图 |
| 配置审批规则 | 进入后能看到规则开关、审批方式、步骤编排和保存状态 | 审批面板截图 |
| 配置菜单入口 | 菜单配置显示真实菜单目录和新增、批量、检查发布分组 | 菜单配置截图 |
| 从子能力返回工作台 | 返回后仍是原业务页面，不让用户重新找页面 | 返回后的标题与卡片 |
| 小屏查看与操作 | 390px 宽度下当前配置先展示，页面无横向溢出 | 移动端截图和宽度指标 |

### 页面结构验收口径

配置工作台桌面端必须形成稳定的四层结构：

1. 顶部说明当前正在配置的业务页面，并提供选择业务页面、配置表单字段、预览页面等主入口。
2. 当前配置区展示表单字段与布局、列表与搜索、菜单入口、审批规则四类任务卡片。
3. 页面目录区用于查找和切换业务页面，不承担配置编辑职责。
