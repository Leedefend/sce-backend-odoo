...
当前基线位于 `scripts/verify/baselines/core_history_field_physical_boundary_budget_v1.json`，`total_hits=0`。该预算只能保持为 0，不能上升。审计同时覆盖直接文本和拆分/格式化构造出的 `legacy_visible` 字段名、`creator_legacy_user_id` 这类历史用户 ID 载体，以及 `legacy_residual_reason` 这类历史残余说明，避免 core/projection 通过字符串拼接或审计字段声明重新耦合历史用户数据。`sc.expense.claim` 与 `sc.financing.loan` 的历史可见字段声明已迁入 `smart_construction_custom`；融资借款的历史往来方 ID、历史金额字段、历史附件引用也已从 core 模型和 core 视图迁出；项目台账历史附件引用已从 core support 迁移承载、core 项目表单、core 表单编排配置和付款执行迁移默认配置迁入 custom 用户历史扩展；`sc.invoice.registration` 的历史附件引用已从 core 字段、core 迁移来源页、core 写保护白名单和 core 业务分类 seed 迁入 custom hook；`sc.construction.diary` 的历史附件引用、明细附件引用、附件名和附件路径已从 core 模型迁入 custom 用户历史扩展；`sc.general.contract` 的历史附件引用已从 core 模型迁入 custom 用户历史扩展；`sc.fund.account.operation` 的历史字段已由 custom 承载，资金账户操作历史附件引用也已从 core 模型、core 迁移来源页和 core 正式配置投影 source mapping 迁出；`sc.settlement.order` 的历史附件引用已从 core 模型和附件计数依赖迁入 custom hook，正式附件数量在 core 仅依赖正式附件关系并通过可覆盖钩子读取历史 fallback；`sc.receipt.income` 的历史附件引用已从 core 模型和 core 列表视图迁入 custom hook，工程进度款收入登记正式列表改用 `receipt_income_attachment_text_display` 展示附件；direct acceptance 共用历史验收载体已从 P1 core 模型目录归类到 support/history carrier。隐藏的历史列回填、正式可见 fallback hook、历史录入人 ID 字段和历史残余说明字段已移到 custom/support，core/projection 不再构造 `legacy_visible_*` 历史字段名，也不再声明 `creator_legacy_user_id` 或 `legacy_residual_reason`。
