...
当前基线位于 `scripts/verify/baselines/core_history_field_physical_boundary_budget_v1.json`，`total_hits=0`。该预算只能保持为 0，不能上升。审计同时覆盖直接文本和拆分/格式化构造出的 `legacy_visible` 字段名，以及 `creator_legacy_user_id` 这类历史用户 ID 载体，避免 core/projection 通过字符串拼接或审计字段声明重新耦合历史用户数据。`sc.expense.claim` 与 `sc.financing.loan` 的历史可见字段声明已迁入 `smart_construction_custom`；`sc.fund.account.operation` 的历史字段已由 custom 承载；direct acceptance 共用历史验收载体已从 P1 core 模型目录归类到 support/history carrier。隐藏的历史列回填、正式可见 fallback hook 和历史录入人 ID 字段已移到 custom/support，core/projection 不再构造 `legacy_visible_*` 历史字段名，也不再声明 `creator_legacy_user_id`。
