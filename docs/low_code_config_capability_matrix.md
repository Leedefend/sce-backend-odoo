...
| 分析视图配置 | `view_orchestration` pivot/graph/calendar/dashboard | `/admin/business-config` 分析视图卡片 / 版本面板 | `ui.business.config.contract` | 后端支持，工作台已可发现分析页面、查看分析视图配置版本并预览页面；复杂编辑器后置 | P5 扩展分析视图编辑器 |
