...
| 客户确认配置沉淀 | `smart_construction_custom` 或对应客户模块 | 已验收的低代码配置结果 / 客户交付清单 | 用户模块 | 规则冻结：低代码是运行时编辑面，用户模块是交付基线；客户确认长期生效的菜单、表单、字段、初始化数据必须沉淀到用户模块，可随安装/升级重放 | P5 补配置沉淀清单和迁移辅助 |
