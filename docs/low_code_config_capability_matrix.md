...
| 配置验收报告 | `ui.business_config.coverage.scan` / `ui.business_config.coverage.bootstrap_missing` / `make verify.business_config.coverage` / `make verify.business_config.full_acceptance` | `/admin/business-config` | 平台工具 | 默认按当前用户可见菜单 action 扫描表单/列表/搜索契约覆盖缺口、运行时有效发布契约缺口及分类原因、菜单入口缺口和个人偏好边界信号；扫描行输出运行页面路由并支持工作台直接打开；工作台支持表单/列表/搜索缺口批量补齐；升级迁移会按系统根菜单和代表角色自动固化缺口；门禁命令按代表角色验证运行态无缺口并落盘运行页面样本；完整验收命令会先构建前端静态资源，再用浏览器批量打开样本页面并输出截图/JSON 证据。系统根菜单扫描只能覆盖标准 ORM 菜单 action，运行时可见菜单仍必须保留代表角色验收 | P5 扩展分析视图 |
