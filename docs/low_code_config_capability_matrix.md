...
| 客户确认配置沉淀 | `smart_construction_custom` 或对应客户模块 | 已验收的低代码配置结果 / 客户交付清单 | 用户模块 | 已补 `lowcode_customer_config_baseline_manifest.v1`，明确菜单偏好、表单偏好、用户数据基线的运行时来源、用户模块承载、安装/升级钩子、快照对比和守卫要求；低代码保存仍只是运行时编辑面，用户模块才是可重放交付基线 | P5 继续补客户模块配置提取/生成辅助 |
