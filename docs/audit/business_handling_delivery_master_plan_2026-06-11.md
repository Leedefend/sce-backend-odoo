...
17. 往来款边界必须从用户数据事实识别，不能从旧菜单名推断。旧系统没有统一往来款概念；旧入口只保留用户认知和追溯证据，是否纳入往来款取决于是否形成公司、项目、承包人或账户之间的资金占用、借出、借入、归还或调拨关系。
18. 资金日报不纳入往来款。后续资金台账口径应承接用户日报、账户收支明细和余额快照；日报台账与往来事实可以在项目资金视角汇合，但不能在事实层互相替代。
...
- 已建立 `make verify.interfund_user_data.full_coverage.audit`，按用户数据事实区分“纳入往来款”和“不纳入但有承接口径”：账户调拨 395、借款事实 872、还款事实 671 全量进入往来事实；资金日报 7453、余额调整 519、到款确认 5205、自筹可见 2144/827、融资登记 152、账户收支/日报明细均不混入往来款。
