...
1. 这次改动属于 L0/L1/L2/L3/L4 哪一层？
2. 写入的权威表是什么？是否只是投影？
3. 是否会覆盖用户管理员已经通过低代码保存的配置？
4. 是否需要 source authority 或 `view_orchestration.context.source`？
5. 是否有运行时配置对象误进入通用表单设计器？
6. 是否需要浏览器验证配置端和业务端两条链路？
