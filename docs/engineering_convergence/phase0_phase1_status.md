...
| GitHub remote execution plan | Done | `github_remote_execution_plan.md` and `github_issue_bodies/*` are generated by `scripts/ci/generate_github_remote_execution_plan.py` and checked by `make ci` |
...
| --- | --- | --- |
| P0-02 Milestone creation | Blocked locally | `gh auth status` reports no authenticated GitHub host; GitHub connector returned `token_expired`; run `github_governance_runbook.md` after re-authentication or through GitHub UI |
...
| Draft PR creation | Blocked remotely | Connector PR creation for `topic/v1.1-engineering-convergence` -> `main` returned HTTP 401 `token_expired`; use `github_remote_execution_plan.md` after GitHub re-authentication |
