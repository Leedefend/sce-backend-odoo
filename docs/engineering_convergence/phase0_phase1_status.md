...
| PR evidence package | Done | `pr_v1_1_engineering_convergence.md` is ready for the GitHub PR body; connector PR creation attempted and blocked by expired GitHub app token |
...
| --- | --- | --- |
| P0-02 Milestone creation | Blocked locally | `gh auth status` reports no authenticated GitHub host; GitHub connector returned `token_expired`; run `github_remote_execute.sh` or `github_governance_runbook.md` after re-authentication |
...
| Draft PR creation | Blocked remotely | Connector PR creation for `topic/v1.1-engineering-convergence` -> `main` returned HTTP 401 `token_expired`; use `github_remote_execute.sh` after GitHub re-authentication |
