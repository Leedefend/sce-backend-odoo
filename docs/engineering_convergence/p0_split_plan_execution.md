...
| `frontend/apps/web/src/pages/ContractFormPage.vue` | P4-P0-03 in progress: shared contract form types/constants, action parsing helpers, contract record helpers, field/date/relation display helpers, access-policy normalization, relation descriptor parsing, one2many pure value helpers, workflow parsing/statusbar helpers, UI label helpers, form-config helpers, native-layout/modifier helpers, and value helpers extracted; route component reduced from 13762 to 12674 lines without changing product behavior. | `@Leedefend` | Local full CI passed |
