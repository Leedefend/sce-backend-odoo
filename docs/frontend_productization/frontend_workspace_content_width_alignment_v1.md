...
| `focused-form` | 只约束字段 canvas，页面头、错误摘要、主 surface 和操作栏仍全宽 |
| `reading` | 只限制长文本阅读行宽 |
...
表单外框、标题、错误摘要、主 surface 和操作栏与列表同边界。`focused-form` 只允许字段 canvas 使用 1080px 内部上限；合同等复杂表单继续使用响应式多列，390px 统一单列。
...
已知兼容边界：旧 `layout.width_mode` 仅映射为内部 Content Layout，不能再改变 Workspace Frame；后端契约应逐步改发 `layout.content_layout_mode`。局部关系表在移动端仍可在自身容器内滚动，但不会越出 router 或产生页面级滚动。
