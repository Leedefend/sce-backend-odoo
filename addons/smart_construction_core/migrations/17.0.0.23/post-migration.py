...
def _map_equipment_price_state(record):
...
def _migrate_equipment_price(env):
    Source = env["sc.equipment.document"].sudo()
    Price = env["sc.equipment.price"].sudo()
    records = Source.search([("fact_type", "=", "equipment_price_library")])
    created = 0
    for record in records:
        ...
        Price.create(
            {
                "name": record.document_no or record.name or "历史设备价格",
                "project_id": record.project_id.id or False,
                "supplier_id": record.partner_id.id or False,
                "equipment_name": record.equipment_name or record.name or "历史设备价格",
                ...
                "state": _map_equipment_price_state(record),
...
def migrate(cr, version):
    ...
    if ICP.get_param("sc.professionalization.v17_0_0_23.done") == "1":
        return
    price_count = _migrate_equipment_price(env)
    ICP.set_param("sc.professionalization.v17_0_0_23.equipment_price_count", str(price_count))
    ICP.set_param("sc.professionalization.v17_0_0_23.done", "1")
