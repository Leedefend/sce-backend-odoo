...
class ScLegacyInvoiceRegistrationLine(models.Model):
    ...
    legacy_line_id = fields.Char(string="历史行ID", required=True, index=True)
    legacy_header_id = fields.Char(string="历史主表ID", index=True)
    legacy_pid = fields.Char(string="历史行PID", index=True)
    legacy_header_pid = fields.Char(string="历史主表PID", index=True)
    project_legacy_id = fields.Char(string="项目历史ID", index=True)
    ...
    fee_project_legacy_id = fields.Char(string="费用项目历史ID", index=True)
    ...
    invoice_type_id = fields.Char(string="发票类型历史ID", index=True)
    supplier_legacy_id = fields.Char(string="供应商历史ID", index=True)
    ...
    tax_rate_id = fields.Char(string="税率历史ID", index=True)
    ...
    contract_legacy_id = fields.Char(string="合同历史ID", index=True)
    settlement_legacy_id = fields.Char(string="结算历史ID", index=True)
    ...
    creator_legacy_user_id = fields.Char(string="创建人历史ID", index=True)
