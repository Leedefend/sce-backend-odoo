    ...
    def _normalize_form_structure_semantics(contract_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for node in UIContractServiceV2._iter_layout_nodes(layout):
            ...
            if node_type == "page":
                ...
                if title:
                    title = UIContractServiceV2._normalize_group_label(title, UIContractServiceV2._collect_group_field_names(node))
                    node["title"] = title
                    node["label"] = title
                continue
            ...
            if node_type == "notebook":
                ...
                for tab_index, tab in enumerate(normalized_tabs, start=1):
                    ...
                    if tab_title:
                        tab_title = UIContractServiceV2._normalize_group_label(tab_title, UIContractServiceV2._collect_group_field_names(tab))
                        tab["title"] = tab_title
                        tab["label"] = tab_title
