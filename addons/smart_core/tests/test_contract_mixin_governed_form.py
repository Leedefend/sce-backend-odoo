# -*- coding: utf-8 -*-
import importlib.util
import sys
import types
import unittest
from pathlib import Path


SMART_CORE_DIR = Path(__file__).resolve().parents[1]
MIXIN_PATH = SMART_CORE_DIR / "app_config_engine" / "models" / "contract_mixin.py"


def _load_module(module_name: str, path: Path):
    spec = importlib.util.spec_from_file_location(module_name, path)
    module = importlib.util.module_from_spec(spec)
    assert spec and spec.loader
    sys.modules[module_name] = module
    spec.loader.exec_module(module)
    return module


sys.modules.setdefault("odoo", types.ModuleType("odoo"))
odoo_module = sys.modules["odoo"]
odoo_module.models = types.SimpleNamespace(AbstractModel=object)


def _identity_decorator(fn):
    return fn


odoo_module.api = types.SimpleNamespace(model=_identity_decorator)

module = _load_module("smart_core_contract_mixin", MIXIN_PATH)
ContractSchemaMixin = module.ContractSchemaMixin


class _DummyMixin(ContractSchemaMixin):
    pass


class TestContractMixinGovernedForm(unittest.TestCase):
    def test_form_governed_sanitize_preserves_field_modifiers(self):
        mixin = _DummyMixin()

        payload = {
            "layout": [{"type": "sheet", "children": []}],
            "statusbar": {"field": "state"},
            "field_modifiers": {
                "name": {"readonly": True},
                "manager_id": {"required": True, "invisible": False},
            },
            "toolbar": {"header": [], "sidebar": [], "footer": []},
            "search": {"filters": [], "group_by": [], "facets": {"enabled": True}},
        }

        result = mixin.sanitize_governed_contract("form", payload)

        self.assertIn("field_modifiers", result)
        self.assertEqual(result["field_modifiers"]["name"]["readonly"], True)
        self.assertEqual(result["field_modifiers"]["manager_id"]["required"], True)


if __name__ == "__main__":
    unittest.main()
