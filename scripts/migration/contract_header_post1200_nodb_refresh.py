...
WRITE_JSONS = [
    ...
    ARTIFACT_DIR / "contract_header_post600_next200_write_result_v1.json",
    ARTIFACT_DIR / "contract_header_post800_next200_write_result_v1.json",
    ARTIFACT_DIR / "contract_header_post1000_next200_write_result_v1.json",
]
ROLLBACK_CSVS = [
...
RESULT_JSON = ARTIFACT_DIR / "contract_header_post1200_nodb_refresh_result_v1.json"
REMAINING_CSV = ARTIFACT_DIR / "contract_header_remaining_after_1200_v1.csv"
NEXT_SLICE_CSV = ARTIFACT_DIR / "contract_header_next_slice200_after_1200_v1.csv"

DATABASE = "sc_demo"
EXPECTED_INPUT_ROWS = 1332
EXPECTED_WRITTEN_ROWS = 1200
EXPECTED_REMAINING_ROWS = 132
...
if len(written_existing_ids) != EXPECTED_WRITTEN_ROWS:
    errors.append({"kind": "existing_written_count_not_1200", "actual": len(written_existing_ids)})
...
if len(remaining_rows) != EXPECTED_REMAINING_ROWS:
    errors.append({"kind": "remaining_candidate_count_not_132", "actual": len(remaining_rows)})
if len(next_slice_rows) != EXPECTED_REMAINING_ROWS:
    errors.append({"kind": "next_slice_count_not_132", "actual": len(next_slice_rows)})
...
result = {
    ...
    "mode": "contract_header_post1200_nodb_refresh",
    ...
    "next_step": "open final 132-row contract header readonly precheck" if not errors else "stop_and_review_errors",
}
...
if errors:
    raise Exception("CONTRACT_HEADER_POST1200_NODB_REFRESH_FAILED=" + json.dumps(result, ensure_ascii=False, sort_keys=True))

print("CONTRACT_HEADER_POST1200_NODB_REFRESH=" + json.dumps({
