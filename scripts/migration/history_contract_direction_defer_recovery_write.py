#!/usr/bin/env python3
"""Replay direction-defer contract headers recovered through repayment strong evidence."""
...
def repo_root() -> Path:
    ...
    for candidate in candidates:
        if (candidate / "artifacts/migration/history_contract_direction_defer_recovery_payload_v1.csv").exists():
            return candidate
    return Path.cwd()
...
def resolve_partner(row, partner_model):
    ...
    if len(partner) > 1:
        return partner[0].id
    if partner_name:
        name_match = partner_model.search([("name", "=", partner_name)], order="id")
        if len(name_match) == 1:
            return name_match.id
        if len(name_match) > 1:
            migration_matches = name_match.filtered(lambda rec: rec.legacy_partner_id or rec.legacy_source_evidence)
            return (migration_matches or name_match)[0].id
    return None


REPO_ROOT = repo_root()
ARTIFACT_ROOT = Path(os.getenv("MIGRATION_ARTIFACT_ROOT", str(REPO_ROOT / "artifacts/migration")))
PAYLOAD_CSV = REPO_ROOT / "artifacts/migration/history_contract_direction_defer_recovery_payload_v1.csv"
OUTPUT_JSON = ARTIFACT_ROOT / "history_contract_direction_defer_recovery_write_result_v1.json"
ROLLBACK_CSV = ARTIFACT_ROOT / "history_contract_direction_defer_recovery_rollback_targets_v1.csv"
EXPECTED_ROWS = 12
...
payload = {
    "status": "PASS",
    "mode": "history_contract_direction_defer_recovery_write",
