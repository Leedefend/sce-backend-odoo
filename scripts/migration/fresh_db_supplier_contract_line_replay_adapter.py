#!/usr/bin/env python3
"""Build replay payload for supplier contract summary lines."""
...
REPO_ROOT = Path.cwd()
INPUT_XML = REPO_ROOT / "migration_assets/20_business/supplier_contract_line/supplier_contract_summary_line_v1.xml"
INPUT_MANIFEST = REPO_ROOT / "migration_assets/manifest/supplier_contract_line_asset_manifest_v1.json"
OUTPUT_JSON = REPO_ROOT / "artifacts/migration/fresh_db_supplier_contract_line_replay_adapter_result_v1.json"
OUTPUT_CSV = REPO_ROOT / "artifacts/migration/fresh_db_supplier_contract_line_replay_payload_v1.csv"
    ...
    payload_rows.append(
        {
            ...
            "legacy_contract_id": contract_ref.removeprefix("legacy_supplier_contract_sc_"),
...
payload = {
    ...
    "decision": "supplier_contract_line_replay_payload_ready" if status == "PASS" else "STOP_REVIEW_REQUIRED",
}
write_csv(
    OUTPUT_CSV,
...
print(
    "FRESH_DB_SUPPLIER_CONTRACT_LINE_REPLAY_ADAPTER="
    + json.dumps(
        {
            "status": status,
            "expected_rows": expected_rows,
