#!/usr/bin/env python3
"""Replay legacy financing loan facts into allowed replay databases."""
...
def repo_root() -> Path:
    ...
    for candidate in candidates:
        if (candidate / "artifacts/migration/fresh_db_legacy_financing_loan_replay_payload_v1.csv").exists():
            return candidate
    return Path.cwd()
...
INPUT_CSV = REPO_ROOT / "artifacts/migration/fresh_db_legacy_financing_loan_replay_payload_v1.csv"
INPUT_JSON = REPO_ROOT / "artifacts/migration/fresh_db_legacy_financing_loan_replay_adapter_result_v1.json"
OUTPUT_JSON = ARTIFACT_ROOT / "fresh_db_legacy_financing_loan_replay_write_result_v1.json"
...
Model = env["sc.legacy.financing.loan.fact"].sudo().with_context(active_test=False)  # noqa: F821
...
partner_master_ids = sorted(
    {
        clean(row.get("partner_ref")).removeprefix("legacy_partner_sc_").replace("_", "-")
        for row in rows
...
receipt_partner_ids = sorted(
    {
        clean(row.get("partner_ref")).removeprefix("legacy_receipt_counterparty_sc_").replace("_", "-")
        for row in rows
        if clean(row.get("partner_ref")).startswith("legacy_receipt_counterparty_sc_")
    }
)
contract_partner_ids = sorted(
    {
        clean(row.get("partner_ref")).removeprefix("legacy_contract_counterparty_sc_").replace("_", "-")
        for row in rows
        if clean(row.get("partner_ref")).startswith("legacy_contract_counterparty_sc_")
    }
)
...
for row in rows:
    ...
    vals = {
        ...
        "source_direction": clean(row.get("source_direction")),
        ...
        "legacy_counterparty_id": clean(row.get("legacy_counterparty_id")),
        "legacy_counterparty_name": clean(row.get("legacy_counterparty_name")),
        ...
        "source_type_label": clean(row.get("source_type_label")),
    ...
    if partner_ref:
        partner_id = partner_map.get(partner_ref)
        if not partner_id:
            raise RuntimeError({"missing_partner_anchor": partner_ref, "external_id": row["external_id"]})
...
payload = {
    "status": status,
    "mode": "fresh_db_legacy_financing_loan_replay_write",
    "database": env.cr.dbname,  # noqa: F821
    "input_rows": len(rows),
    "created_rows": created,
    "skipped_existing": skipped,
    "db_writes": created,
    "decision": "legacy_financing_loan_replay_write_complete" if status == "PASS" else "STOP_REVIEW_REQUIRED",
}
write_json(OUTPUT_JSON, payload)
print("FRESH_DB_LEGACY_FINANCING_LOAN_REPLAY_WRITE=" + json.dumps(payload, ensure_ascii=False, sort_keys=True))
