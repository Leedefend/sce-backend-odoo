#!/usr/bin/env python3
"""Build a no-DB create-only design for the post1000 next 200-row contract header slice."""
...
REPO_ROOT = Path.cwd()
INPUT_CSV = REPO_ROOT / "artifacts/migration/contract_header_next_slice200_after_1000_v1.csv"
PRECHECK_JSON = REPO_ROOT / "artifacts/migration/contract_header_post1000_next200_readonly_precheck_result_v1.json"
OUTPUT_CSV = REPO_ROOT / "artifacts/migration/contract_header_post1000_next200_write_design_rows_v1.csv"
OUTPUT_JSON = REPO_ROOT / "artifacts/migration/contract_header_post1000_next200_write_design_result_v1.json"
...
def main():
    ...
    for index, row in enumerate(rows, start=1):
        ...
        design.update(
            {
                "write_design_id": f"contract_header_post1000_next200:{index:04d}",
    ...
    payload = {
        ...
        "mode": "contract_header_post1000_next200_write_design_no_db",
        ...
        "next_step": "open no-DB authorization packet for the post1000 next 200-row design" if len(design_rows) == len(rows) and not blockers else "resolve design blockers",
    }
    write_json(OUTPUT_JSON, payload)
    print(
        "CONTRACT_HEADER_POST1000_NEXT200_WRITE_DESIGN="
        + json.dumps(
            {
