"""Readonly review for same-tax company/supplier canonical partner write."""
...
DESIGN_CSV = Path("/mnt/artifacts/migration/partner_l4_same_tax_company_supplier_canonical_write_design_rows_v1.csv")
ROLLBACK_TARGET_CSV = Path("/mnt/artifacts/migration/partner_l4_same_tax_company_supplier_canonical_102_rollback_targets_v1.csv")
WRITE_RESULT_JSON = Path("/mnt/artifacts/migration/partner_l4_same_tax_company_supplier_canonical_102_write_result_v1.json")
OUTPUT_JSON = Path("/mnt/artifacts/migration/partner_l4_same_tax_company_supplier_canonical_102_post_write_review_result_v1.json")
EXPECTED_COUNT = 102
...
def main():
    ...
    for target in target_rows:
        ...
        if legacy_source != LEGACY_SOURCE or legacy_id not in design_id_set:
            out_of_scope_targets.append({"partner_id": partner_id, "legacy_partner_source": legacy_source, "legacy_partner_id": legacy_id})
        if rec and (rec.customer_rank != 1 or rec.supplier_rank != 1):
    ...
    payload = {
        ...
        "mode": "partner_same_tax_company_supplier_canonical_post_write_readonly_review",
        ...
        "rollback_eligible_rows": rollback_eligible,
    ...
    print("PARTNER_SAME_TAX_COMPANY_SUPPLIER_CANONICAL_POST_WRITE_REVIEW=" + json.dumps({"status": payload["status"], "design_rows": payload["design_rows"], "rollback_target_rows": payload["rollback_target_rows"], "matched_partner_rows": payload["matched_partner_rows"], "rollback_eligible_rows": payload["rollback_eligible_rows"], "blocking_reasons": payload["blocking_reasons"]}, ensure_ascii=False, sort_keys=True))
    if blocking_reasons:
        raise RuntimeError({"partner_same_tax_company_supplier_canonical_post_write_review_failed": blocking_reasons})
