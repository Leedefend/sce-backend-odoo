"""Bounded write for 500 duplicate-relation project_member neutral rows."""
...
GATE_JSON = Path("/mnt/artifacts/migration/project_member_duplicate_relation_write_gate_v1.json")
...
PRE_VISIBILITY_JSON = Path("/mnt/artifacts/migration/project_member_duplicate_relation_500_pre_visibility_v1.json")
POST_VISIBILITY_JSON = Path("/mnt/artifacts/migration/project_member_duplicate_relation_500_post_visibility_v1.json")
WRITE_RESULT_JSON = Path("/mnt/artifacts/migration/project_member_duplicate_relation_500_write_result_v1.json")
ROLLBACK_TARGET_CSV = Path("/mnt/artifacts/migration/project_member_duplicate_relation_500_rollback_targets_v1.csv")

RUN_ID = "ITER-2026-04-14-0030NZ"
...
def main():
    ...
    if gate.get("status") != "PASS":
        raise RuntimeError({"duplicate_relation_gate_not_pass": gate.get("status")})
    ...
    payload = {
        ...
        "mode": "project_member_duplicate_relation_neutral_500_write",
    ...
    print("PROJECT_MEMBER_DUPLICATE_RELATION_NEUTRAL_500_WRITE=" + json.dumps({
    ...
    if post_errors:
        raise RuntimeError({"duplicate_relation_neutral_write_failed": post_errors})
