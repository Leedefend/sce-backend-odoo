#!/usr/bin/env python3
"""Build replay payload for legacy receipt income facts."""
...
INPUT_XML = REPO_ROOT / "migration_assets/30_relation/legacy_receipt_income/legacy_receipt_income_v1.xml"
OUTPUT_CSV = REPO_ROOT / "artifacts/migration/fresh_db_legacy_receipt_income_replay_payload_v1.csv"
OUTPUT_JSON = REPO_ROOT / "artifacts/migration/fresh_db_legacy_receipt_income_replay_adapter_result_v1.json"
...
def main() -> int:
    ...
    for _, elem in ET.iterparse(INPUT_XML, events=("end",)):
        if elem.tag != "record":
            continue
        if clean(elem.attrib.get("model")) != "sc.legacy.receipt.income.fact":
            elem.clear()
            continue
