# -*- coding: utf-8 -*-
"""Project accepted direct-project equipment contracts into formal expense execution."""
...
ACCEPTANCE_LABEL = "机械合同（合同）"
TARGET_MODEL = "construction.contract"
TARGET_WRAPPER_MODEL = "construction.contract.expense"
LEGACY_ID_PREFIX = "direct_acceptance:equipment_contract:"
MIGRATION_MARKER = "[migration:direct_acceptance_equipment_contract_to_expense_execution]"
...
OUTPUT_JSON = artifact_root() / "direct_acceptance_equipment_contract_expense_execution_write_result_v1.json"
...
def resolve_project(fact, payload: dict):
    ...
    project_name = text(fact.legacy_visible_02) or text(fact.project_name) or first_text(payload, "XMMC", "ProjectName")
    if not project_name:
        project_name = "直营项目机械合同未匹配项目"
...
def resolve_partner(fact, payload: dict):
    ...
    name = text(fact.legacy_visible_03) or first_text(payload, "FBDW", "ZLDW", "JSDW") or text(fact.partner_name)
    if not name:
        name = "直营项目机械合同未匹配单位"
    ...
    if "legacy_partner_id" in Partner._fields:
        vals["legacy_partner_id"] = "direct_acceptance_equipment:" + name
    if "legacy_partner_source" in Partner._fields:
        vals["legacy_partner_source"] = "direct_acceptance_equipment_contract"
...
def machine_category():
    category = env.ref("smart_construction_core.dict_expense_contract_category_machine", raise_if_not_found=False)  # noqa: F821
    if category:
        return category
    return env["sc.dictionary"].sudo().search(  # noqa: F821
        [("type", "=", "expense_contract_category"), ("code", "=", "machine")],
        limit=1,
    )
...
def attachment_display_and_ref(fact, payload: dict) -> tuple[str, str]:
    visible_attachment = text(fact.legacy_visible_51)
    raw_ref = text(fact.attachment_ref)
    if raw_ref and not raw_ref.startswith("附件("):
        return visible_attachment, raw_ref
    payload_ref = first_text(payload, "f_FJ", "FJ")
    if payload_ref and not payload_ref.startswith("附件("):
        return visible_attachment or payload_ref, payload_ref
    return visible_attachment or payload_ref, ""
...
def contract_values(fact, category):
    ...
    visible_project_name = text(fact.legacy_visible_02)
    visible_partner = text(fact.legacy_visible_03)
    visible_contract_no = text(fact.legacy_visible_04)
    visible_content = text(fact.legacy_visible_05)
    visible_method = text(fact.legacy_visible_06)
    visible_duration = text(fact.legacy_visible_07)
    visible_stamp_tax = text(fact.legacy_visible_08)
    visible_amount = text(fact.legacy_visible_09)
    visible_manager = text(fact.legacy_visible_10)
    visible_sign_date = text(fact.legacy_visible_11)
    visible_return_date = text(fact.legacy_visible_12)
    visible_note_1 = text(fact.legacy_visible_13)
    visible_handler_phone = text(fact.legacy_visible_14)
    visible_document_no = text(fact.legacy_visible_18) or text(fact.document_no)
    visible_bank_account = text(fact.legacy_visible_20)
    visible_bank_name = text(fact.legacy_visible_21)
    visible_bank_holder = text(fact.legacy_visible_22)
    visible_quantity = text(fact.legacy_visible_23)
    visible_total_amount = text(fact.legacy_visible_24)
    visible_contract_type = text(fact.legacy_visible_25)
    visible_work_part = text(fact.legacy_visible_27)
    visible_title = text(fact.legacy_visible_28) or text(fact.legacy_visible_47)
    visible_signer = text(fact.legacy_visible_29)
    visible_department = text(fact.legacy_visible_30)
    visible_own_company = text(fact.legacy_visible_31)
    visible_payee = text(fact.legacy_visible_32)
    visible_currency = text(fact.legacy_visible_33)
    visible_rate = text(fact.legacy_visible_34)
    visible_amount_no_tax = text(fact.legacy_visible_35)
    visible_tax_amount = text(fact.legacy_visible_36)
    visible_tax_rate = text(fact.legacy_visible_37)
    visible_tax_type = text(fact.legacy_visible_40)
    visible_note_2 = text(fact.legacy_visible_41)
    visible_pricing_method = text(fact.legacy_visible_42)
    visible_project_manager_name = text(fact.legacy_visible_45)
    visible_seal = text(fact.legacy_visible_46)
    visible_settlement_method = text(fact.legacy_visible_48)
    visible_push_project = text(fact.legacy_visible_49)
    visible_fund_balance = text(fact.legacy_visible_50)
    visible_attachment, attachment_ref = attachment_display_and_ref(fact, payload)
    visible_creator = text(fact.legacy_visible_52) or text(fact.creator_name) or first_text(payload, "LRR", "f_LRR", "XGR")
    visible_created_time = text(fact.legacy_visible_53) or str(fact.created_time or "") or first_text(payload, "LRSJ", "f_LRSJ", "XGSJ")
    amount_source_text = visible_amount or text(fact.amount_total) or first_text(payload, "JE")
    amount_total = parse_amount(amount_source_text)
    accepted_title = visible_title or text(fact.document_title) or visible_document_no or text(fact.legacy_record_id)
    content_text = visible_content or visible_title or text(fact.document_title) or accepted_title
    subject = execution_title(accepted_title or content_text)
    attachment_text = visible_attachment
    if attachment_ref:
        attachment_text = (attachment_text + "\n" if attachment_text else "") + "legacy-file-id://" + attachment_ref
    user_note = visible_note_1 or visible_note_2
    vals = {
        "legacy_contract_id": legacy_contract_id(fact),
        "legacy_project_id": text(fact.project_legacy_id) or first_text(payload, "XMID", "SJBXXMID"),
        "legacy_contract_no": visible_contract_no,
        "legacy_document_no": visible_document_no,
        "legacy_external_contract_no": visible_document_no,
        "legacy_status": visible_state or first_text(payload, "DJZT", "DJZTText"),
        "legacy_counterparty_text": visible_partner,
        "legacy_contract_amount_source": SOURCE_MODEL + ".legacy_visible_09",
        ...
        "legacy_visible_counterparty": visible_partner,
        ...
        "legacy_visible_category": "机械",
        "legacy_visible_contract_no": visible_contract_no,
        "legacy_visible_contract_date": parse_date(visible_sign_date),
        "legacy_visible_amount": amount_source_text,
        "legacy_visible_engineering_content": content_text or visible_work_part,
        ...
        "date_contract": parse_date(visible_sign_date),
        "engineering_content": content_text or visible_work_part,
        "contract_duration_text": visible_duration,
        "contract_payment_method_text": visible_settlement_method,
        "entry_user_text": visible_creator,
        "entry_time": parse_datetime(visible_created_time),
        "attachment_text": attachment_text,
        "note": user_note or False,
    }
    if visible_contract_type:
        vals["engineering_category_text"] = visible_contract_type
    if category:
        vals["expense_contract_category_id"] = category.id
    if visible_own_company:
        vals["legacy_visible_contractor"] = visible_own_company
...
def main() -> None:
    ...
    if not category:
        raise RuntimeError({"missing_expense_contract_machine_category": True})
        ...
        detail.append(
            {
                ...
                "creator": contract.legacy_visible_creator_name,
    ...
    machine_count = Contract.search_count(projected_domain + [("expense_contract_category_id", "=", category.id)])
    ...
    result = {
        ...
        and machine_count == len(seen)
        and direct_count == len(seen)
        and creator_count == len(seen)
        else "FAIL",
        "mode": "direct_acceptance_equipment_contract_expense_execution_write",
        ...
        "machine_category_count": machine_count,
    ...
    print("DIRECT_ACCEPTANCE_EQUIPMENT_CONTRACT_EXPENSE_EXECUTION_WRITE=" + json.dumps(result, ensure_ascii=False, sort_keys=True))
