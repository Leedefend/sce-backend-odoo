#!/usr/bin/env python3
"""Verify repository-tracked supplier contract XML migration assets."""
...
REQUIRED_FIELDS = {"legacy_contract_id", "legacy_project_id", "subject", "type", "project_id", "partner_id", "note"}
ALLOWED_XML_FIELDS = {
    "legacy_contract_id",
    "legacy_project_id",
    "legacy_document_no",
...
class SupplierContractAssetVerificationError(Exception):
    pass


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SupplierContractAssetVerificationError(message)
...
def verify_asset_manifest(asset_manifest: dict[str, Any], lane: str) -> None:
    ...
    require(asset_manifest.get("asset_package_id") == "supplier_contract_sc_v1", "unexpected supplier contract package id")
    ...
    require(asset_manifest.get("target", {}).get("type") == "in", "unexpected target type")


def verify_xml_records(records: list[dict[str, str]], expected_count: int, project_refs: set[str], partner_refs: set[str]) -> None:
    ...
    for row in records:
        require(row["id"].startswith("legacy_supplier_contract_sc_"), f"invalid external id: {row['id']}")
        ...
        require(not unexpected, f"unexpected supplier contract fields for {row['id']}: {unexpected}")
        forbidden = sorted(FORBIDDEN_XML_FIELDS & set(row))
        require(not forbidden, f"forbidden supplier contract fields for {row['id']}: {forbidden}")
        require(row["type"] == "in", f"type must be in for {row['id']}")
        ...
        require("[migration:supplier_contract_header]" in row["note"], f"missing migration note marker for {row['id']}")
        require("type_in_supplier_expense=true" in row["note"], f"missing type semantic marker for {row['id']}")
        require("amount_header_not_written=true" in row["note"], f"missing amount boundary marker for {row['id']}")
...
def verify_external_manifest(external_manifest: dict[str, Any], records: list[dict[str, str]]) -> None:
    ...
    for row in manifest_records:
        ...
        require(row.get("target_type") == "in", f"invalid target type: {row}")
...
def verify_validation_manifest(validation_manifest: dict[str, Any]) -> None:
    ...
    require(boundary.get("carrier") == "type_in_supplier_expense", "carrier must be type_in_supplier_expense")
    require(boundary.get("header_amount_write") == "excluded", "header amount write must be excluded")
    require(boundary.get("tax_write") == "excluded", "tax write must be excluded")
    require(boundary.get("state_replay") == "excluded", "state replay must be excluded")
    require(boundary.get("line_write") == "excluded", "line write must be excluded")


def verify(asset_root: Path, lane: str) -> dict[str, Any]:
    asset_manifest = load_json(asset_root / "manifest" / "supplier_contract_asset_manifest_v1.json")
    external_manifest = load_json(asset_root / "manifest" / "supplier_contract_external_id_manifest_v1.json")
    validation_manifest = load_json(asset_root / "manifest" / "supplier_contract_validation_manifest_v1.json")
    records = parse_xml(asset_root / "20_business" / lane / "supplier_contract_header_v1.xml")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify supplier contract XML migration assets.")
    parser.add_argument("--asset-root", default="migration_assets")
    parser.add_argument("--lane", default="supplier_contract")
    parser.add_argument("--check", action="store_true")
    args = parser.parse_args()

    try:
        result = verify(Path(args.asset_root), args.lane)
    except (SupplierContractAssetVerificationError, ET.ParseError, json.JSONDecodeError) as exc:
        payload = {"status": "FAIL", "error": str(exc), "db_writes": 0, "odoo_shell": False}
        print("SUPPLIER_CONTRACT_ASSET_VERIFY=" + json.dumps(payload, ensure_ascii=False, sort_keys=True))
        return 1 if args.check else 0
    print("SUPPLIER_CONTRACT_ASSET_VERIFY=" + json.dumps(result, ensure_ascii=False, sort_keys=True))
    return 0
