"""Readonly review for remaining missing-tax company/supplier partner write."""
...
OUTPUT_JSON = Path("/mnt/artifacts/migration/partner_l4_remaining_missing_tax_company_supplier_652_post_write_review_result_v1.json")
EXPECTED_COUNT = 652
...
def main():
    ...
    for target in target_rows:
        ...
        rows.append({"partner_id": partner_id, "rollback_eligible": bool(rec) and legacy_source == LEGACY_SOURCE and legacy_id in design_id_set and len(matches) == 1})
    ...
    payload = {
        ...
        "mode": "partner_remaining_missing_tax_company_supplier_post_write_readonly_review",
    ...
    print("PARTNER_REMAINING_MISSING_TAX_COMPANY_SUPPLIER_POST_WRITE_REVIEW=" + json.dumps({"status": payload["status"], "design_rows": payload["design_rows"], "rollback_target_rows": payload["rollback_target_rows"], "matched_partner_rows": payload["matched_partner_rows"], "rollback_eligible_rows": payload["rollback_eligible_rows"], "blocking_reasons": payload["blocking_reasons"]}, ensure_ascii=False, sort_keys=True))
    if blocking_reasons:
        raise RuntimeError({"partner_remaining_missing_tax_company_supplier_post_write_review_failed": blocking_reasons})
