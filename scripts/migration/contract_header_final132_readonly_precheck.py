#!/usr/bin/env python3
"""Readonly DB precheck for the final 132-row contract header slice."""
...
OUTPUT_JSON = REPO_ROOT / "artifacts/migration/contract_header_final132_readonly_precheck_result_v1.json"
...
def main():
    ...
    payload = {
        ...
        "mode": "contract_header_final132_readonly_precheck",
        "database": DB_NAME,
        ...
        "rows": row_results[:30],
        "next_step": "open no-DB write design for the final 132-row slice" if ready_rows == len(row_results) else "resolve precheck blockers before write design",
    }
    write_json(OUTPUT_JSON, payload)
    print(
        "CONTRACT_HEADER_FINAL132_READONLY_PRECHECK="
        + json.dumps(
            {
