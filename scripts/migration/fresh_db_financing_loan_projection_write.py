...
env.cr.execute(  # noqa: F821
    ...
    INSERT INTO sc_financing_loan (
      ...
      financing_loan_request_time, financing_loan_actual_loan_amount,
      financing_loan_fund_usage_plan, financing_loan_borrow_account,
      financing_loan_approved_amount, financing_loan_request_amount,
      financing_loan_expected_return_time, financing_loan_loan_type_display,
    ...
    SELECT
      ...
      %s,
      NULLIF(f.purpose, ''),
      COALESCE(f.created_time::text, f.document_date::text, ''),
      NULLIF(GREATEST(COALESCE(f.source_amount, 0), 0)::text, '0'),
      NULLIF(f.purpose, ''),
      NULLIF(f.source_extra_ref, ''),
      NULLIF(GREATEST(COALESCE(f.source_amount, 0), 0)::text, '0'),
      NULLIF(GREATEST(COALESCE(f.source_amount, 0), 0)::text, '0'),
      COALESCE(f.due_date::text, ''),
      COALESCE(NULLIF(f.source_type_label, ''), CASE WHEN f.source_family = 'borrowing_request' THEN '借款申请' ELSE '贷款登记' END),
    ...
    DO UPDATE SET
      ...
      financing_loan_request_time = EXCLUDED.financing_loan_request_time,
      financing_loan_actual_loan_amount = EXCLUDED.financing_loan_actual_loan_amount,
      financing_loan_fund_usage_plan = EXCLUDED.financing_loan_fund_usage_plan,
      financing_loan_borrow_account = EXCLUDED.financing_loan_borrow_account,
      financing_loan_approved_amount = EXCLUDED.financing_loan_approved_amount,
      financing_loan_request_amount = EXCLUDED.financing_loan_request_amount,
      financing_loan_expected_return_time = EXCLUDED.financing_loan_expected_return_time,
      financing_loan_loan_type_display = EXCLUDED.financing_loan_loan_type_display,
