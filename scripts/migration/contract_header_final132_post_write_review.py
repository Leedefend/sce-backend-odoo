"""Readonly post-write review for the final 132-row contract header write batch."""
...
OUTPUT_JSON = Path("/mnt/artifacts/migration/contract_header_final132_post_write_review_result_v1.json")
EXPECTED_COUNT = 132
...
def main():
    ...
    target_rows = read_csv(ROLLBACK_TARGET_CSV)
    ...
    if len(target_rows) != EXPECTED_COUNT:
        blocking_reasons.append("rollback_target_not_132_rows")
    if len(records) != EXPECTED_COUNT:
        blocking_reasons.append("matched_contract_rows_not_132")
    if sorted(key for key, count in Counter(legacy_ids).items() if key and count > 1):
    ...
    payload = {
        ...
        "mode": "contract_header_final132_post_write_readonly_review",
    ...
    print(
        "CONTRACT_HEADER_FINAL132_POST_WRITE_REVIEW="
        + json.dumps(
            {
    ...
    if blocking_reasons:
        raise RuntimeError({"contract_header_final132_post_write_review_failed": blocking_reasons})
