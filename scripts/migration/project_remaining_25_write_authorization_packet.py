"""Build no-DB authorization packet for remaining project 25-row candidate."""
...
CANDIDATE_CSV = Path("artifacts/migration/project_remaining_candidate_v6.csv")
DRY_RUN_JSON = Path("artifacts/migration/project_remaining_dry_run_result_v6.json")
OUTPUT_JSON = Path("artifacts/migration/project_remaining_25_write_authorization_packet.json")
TARGET_COUNT = 25
SAFE_FIELDS = [
...
if dry_run.get("summary", {}).get("create") != TARGET_COUNT:
    blockers.append("create_count_not_25")
...
if len(rows) != TARGET_COUNT:
    blockers.append("candidate_not_25_rows")
if missing:
...
print("PROJECT_REMAINING_25_WRITE_AUTH_PACKET=" + json.dumps({"status": payload["status"], "payload_rows": payload["payload_rows"], "blockers": payload["blockers"], "write_authorization": payload["write_authorization"]}, ensure_ascii=False, sort_keys=True))
