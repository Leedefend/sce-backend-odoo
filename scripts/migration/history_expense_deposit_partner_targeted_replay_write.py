#!/usr/bin/env python3
"""Replay targeted expense-deposit partner anchors into allowed replay databases."""
...
def repo_root() -> Path:
    ...
    for candidate in candidates:
        if (candidate / "artifacts/migration/history_expense_deposit_partner_targeted_replay_payload_v1.csv").exists():
            return candidate
    return Path.cwd()


def ensure_allowed_db() -> None:
    allowlist = {item.strip() for item in os.getenv("MIGRATION_REPLAY_DB_ALLOWLIST", "sc_migration_fresh,sc_demo").split(",") if item.strip()}
...
INPUT_CSV = REPO_ROOT / "artifacts/migration/history_expense_deposit_partner_targeted_replay_payload_v1.csv"
OUTPUT_JSON = ARTIFACT_ROOT / "history_expense_deposit_partner_targeted_replay_write_result_v1.json"
...
payload = {
    "status": "PASS",
    "mode": "history_expense_deposit_partner_targeted_replay_write",
...
print("HISTORY_EXPENSE_DEPOSIT_PARTNER_TARGETED_REPLAY_WRITE=" + json.dumps(payload, ensure_ascii=False, sort_keys=True))
