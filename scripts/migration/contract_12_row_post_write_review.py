"""Readonly post-write review for the 12-row contract write batch."""
...
OUTPUT_JSON = Path("/mnt/artifacts/migration/contract_12_row_post_write_review_result_v1.json")
EXPECTED_COUNT = 12
...
def main():
    ...
    for target in target_rows:
        ...
        if legacy_contract_id not in legacy_ids:
            out_of_scope_targets.append({"contract_id": contract_id, "legacy_contract_id": legacy_contract_id})
        line_count = len(rec.line_ids) if rec else None
        payment_request_count = rec.payment_request_count if rec else None
        settlement_count = rec.settlement_count if rec else None
        rollback_eligible = (
            bool(rec)
            and len(matches) == 1
            and rec.state == "draft"
            and line_count == 0
            and payment_request_count == 0
    ...
    if len(target_rows) != EXPECTED_COUNT:
        blocking_reasons.append("rollback_target_not_12_rows")
    if len(records) != EXPECTED_COUNT:
        blocking_reasons.append("matched_contract_rows_not_12")
    ...
    payload = {
        ...
        "mode": "contract_12_row_post_write_readonly_review",
    ...
    print(
        "CONTRACT_12_ROW_POST_WRITE_REVIEW="
        + json.dumps(
            {
    ...
    if blocking_reasons:
        raise RuntimeError({"contract_post_write_review_failed": blocking_reasons})
