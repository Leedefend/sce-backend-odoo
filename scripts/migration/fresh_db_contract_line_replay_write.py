#!/usr/bin/env python3
"""Replay contract summary lines into allowed replay databases."""
...
INPUT_CSV = REPO_ROOT / "artifacts/migration/fresh_db_contract_line_replay_payload_v1.csv"
INPUT_MANIFEST = REPO_ROOT / "artifacts/migration/fresh_db_contract_line_replay_adapter_result_v1.json"
OUTPUT_JSON = ARTIFACT_ROOT / "fresh_db_contract_line_replay_write_result_v1.json"
...
for row in rows:
    ...
    if not contract:
        raise RuntimeError({"missing_parent_contract": row["legacy_contract_id"], "external_id": row["external_id"]})
...
payload = {
    "status": status,
    "mode": "fresh_db_contract_line_replay_write",
    "database": env.cr.dbname,  # noqa: F821
    "input_rows": len(rows),
    "created_rows": created,
    "skipped_existing": skipped,
    "db_writes": created,
    "decision": "contract_line_replay_write_complete" if status == "PASS" else "STOP_REVIEW_REQUIRED",
}
write_json(OUTPUT_JSON, payload)
print("FRESH_DB_CONTRACT_LINE_REPLAY_WRITE=" + json.dumps(payload, ensure_ascii=False, sort_keys=True))
