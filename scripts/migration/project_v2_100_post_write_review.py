"""Readonly post-write review for the project v2 100-row write batch."""
...
POST_WRITE_SNAPSHOT_CSV = Path("/mnt/artifacts/migration/project_v2_100_post_write_snapshot.csv")
WRITE_RESULT_JSON = Path("/mnt/artifacts/migration/project_v2_100_write_result.json")
OUTPUT_JSON = Path("/mnt/artifacts/migration/project_v2_100_post_write_review_result.json")
ROLLBACK_TARGET_CSV = Path("/mnt/artifacts/migration/project_v2_100_rollback_target_list.csv")
EXPECTED_COUNT = 100
...
def main():
    ...
    if len(snapshot_rows) != EXPECTED_COUNT:
        blocking_reasons.append("post_write_snapshot_not_100_rows")
    if len(records) != EXPECTED_COUNT:
        blocking_reasons.append("matched_project_rows_not_100")
    ...
    payload = {
        ...
        "mode": "project_v2_100_post_write_readonly_review",
    ...
    print(
        "PROJECT_V2_100_POST_WRITE_REVIEW="
        + json.dumps(
            {
    ...
    if blocking_reasons:
        raise RuntimeError({"project_v2_post_write_review_failed": blocking_reasons})
