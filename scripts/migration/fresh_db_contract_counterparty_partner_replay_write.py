#!/usr/bin/env python3
"""Replay contract counterparty partner anchors into allowed replay databases."""
...
INPUT_CSV = REPO_ROOT / "artifacts/migration/fresh_db_contract_counterparty_partner_replay_payload_v1.csv"
INPUT_MANIFEST = REPO_ROOT / "artifacts/migration/fresh_db_contract_counterparty_partner_replay_adapter_result_v1.json"
OUTPUT_JSON = ARTIFACT_ROOT / "fresh_db_contract_counterparty_partner_replay_write_result_v1.json"
ROLLBACK_CSV = ARTIFACT_ROOT / "fresh_db_contract_counterparty_partner_replay_rollback_targets_v1.csv"
...
for index, row in enumerate(rows, start=1):
    if row.get("legacy_partner_source") != "contract_counterparty":
        errors.append({"line": index, "error": "unexpected_source", "value": row.get("legacy_partner_source")})
    if row.get("company_type") != "company":
...
result = {
    "status": status,
    "mode": "fresh_db_contract_counterparty_partner_replay_write",
    ...
    "decision": "contract_counterparty_partner_replay_write_complete" if status == "PASS" else "STOP_REVIEW_REQUIRED",
    "next_step": "replay receipt counterparty partner anchors",
}
write_csv(
    ROLLBACK_CSV,
...
print(
    "FRESH_DB_CONTRACT_COUNTERPARTY_PARTNER_REPLAY_WRITE="
    + json.dumps(
        {
            "status": status,
            "input_rows": len(rows),
