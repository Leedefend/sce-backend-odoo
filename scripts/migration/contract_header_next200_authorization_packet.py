#!/usr/bin/env python3
"""Build a no-DB authorization packet for the next 200-row contract header design."""
...
REPO_ROOT = Path.cwd()
DESIGN_JSON = REPO_ROOT / "artifacts/migration/contract_header_next200_write_design_result_v1.json"
DESIGN_CSV = REPO_ROOT / "artifacts/migration/contract_header_next200_write_design_rows_v1.csv"
OUTPUT_CSV = REPO_ROOT / "artifacts/migration/contract_header_next200_authorization_payload_v1.csv"
OUTPUT_JSON = REPO_ROOT / "artifacts/migration/contract_header_next200_authorization_packet_v1.json"
...
def main():
    ...
    packet = {
        ...
        "mode": "contract_header_next200_authorization_packet_no_db",
        "database": "sc_demo",
        "db_writes": 0,
        ...
        "next_step": "open dedicated DB write batch for the next 200-row payload" if len(payload_rows) == 200 and not blockers else "resolve packet blockers",
    }
    write_json(OUTPUT_JSON, packet)
    print(
        "CONTRACT_HEADER_NEXT200_AUTH_PACKET="
        + json.dumps(
            {
