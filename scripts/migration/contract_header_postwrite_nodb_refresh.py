...
WRITE_RESULT_JSON = ARTIFACT_DIR / "contract_header_slice200_write_result_v1.json"
ROLLBACK_TARGETS_CSV = ARTIFACT_DIR / "contract_header_slice200_rollback_targets_v1.csv"
RESULT_JSON = ARTIFACT_DIR / "contract_header_postwrite_nodb_refresh_result_v1.json"
REMAINING_CSV = ARTIFACT_DIR / "contract_header_remaining_after_slice200_v1.csv"
NEXT_SLICE_CSV = ARTIFACT_DIR / "contract_header_next_slice200_after_first_write_v1.csv"

DATABASE = "sc_demo"
EXPECTED_INPUT_ROWS = 1332
EXPECTED_WRITTEN_ROWS = 200
EXPECTED_REMAINING_ROWS = 1132
...
rollback_rows = read_csv_rows(ROLLBACK_TARGETS_CSV)
with WRITE_RESULT_JSON.open("r", encoding="utf-8") as handle:
    write_result = json.load(handle)
...
if write_result.get("status") != "PASS" or int(write_result.get("created") or 0) != EXPECTED_WRITTEN_ROWS:
    errors.append({"kind": "upstream_write_not_pass", "write_result": write_result})
...
if len(written_existing_ids) != EXPECTED_WRITTEN_ROWS:
    errors.append({"kind": "existing_written_slice_count_not_200", "actual": len(written_existing_ids)})
...
if len(remaining_rows) != EXPECTED_REMAINING_ROWS:
    errors.append({"kind": "remaining_candidate_count_not_1132", "actual": len(remaining_rows)})
...
result = {
    ...
    "mode": "contract_header_postwrite_nodb_refresh",
    "database": env.cr.dbname,
    "db_writes": 0,
    "input_rows": len(input_rows),
    "upstream_created_rows": int(write_result.get("created") or 0),
...
if errors:
    raise Exception("CONTRACT_HEADER_POSTWRITE_NODB_REFRESH_FAILED=" + json.dumps(result, ensure_ascii=False, sort_keys=True))

print("CONTRACT_HEADER_POSTWRITE_NODB_REFRESH=" + json.dumps({
