#!/usr/bin/env python3
"""Verify supplier contract summary line XML migration assets."""
...
REQUIRED_FIELDS = {"contract_id", "sequence", "qty_contract", "price_contract", "note"}
ALLOWED_XML_FIELDS = {"contract_id", "sequence", "qty_contract", "price_contract", "note"}
FORBIDDEN_XML_FIELDS = {
    "amount_contract",
    "amount_contract_leaf",
    "boq_amount_leaf",
    "delta_amount",
    "boq_amount_source",
    "project_id",
...
class SupplierContractLineAssetVerificationError(Exception):
    pass


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SupplierContractLineAssetVerificationError(message)
...
def verify_asset_manifest(asset_manifest: dict[str, Any], lane: str) -> None:
    ...
    require(asset_manifest.get("asset_package_id") == "supplier_contract_line_sc_v1", "unexpected package id")
    ...
    require(asset_manifest.get("dependencies") == ["supplier_contract_sc_v1"], "unexpected dependencies")
    ...
    require(asset_manifest.get("target", {}).get("model") == "construction.contract.line", "unexpected target model")


def verify_xml_records(records: list[dict[str, str]], expected_count: int, contract_refs: set[str]) -> None:
    ...
    for row in records:
        require(row["id"].startswith("legacy_supplier_contract_line_sc_"), f"invalid external id: {row['id']}")
        require(row["model"] == "construction.contract.line", f"invalid XML model: {row['model']}")
        ...
        require(not unexpected, f"unexpected line fields for {row['id']}: {unexpected}")
        forbidden = sorted(FORBIDDEN_XML_FIELDS & set(row))
        require(not forbidden, f"forbidden computed/related line fields for {row['id']}: {forbidden}")
        require(row["contract_id"] in contract_refs, f"contract ref does not resolve: {row['contract_id']}")
        require(Decimal(row["qty_contract"]) == Decimal("1"), f"qty_contract must be 1 for {row['id']}")
        require(positive_decimal(row["price_contract"]), f"price_contract must be positive for {row['id']}")
        require("summary_amount_line=true" in row["note"], f"missing summary marker for {row['id']}")
        require("qty_contract_one=true" in row["note"], f"missing qty policy marker for {row['id']}")
        require("price_contract_source_amount=true" in row["note"], f"missing price policy marker for {row['id']}")
        require("header_amount_not_written=true" in row["note"], f"missing header boundary marker for {row['id']}")
...
def verify_external_manifest(external_manifest: dict[str, Any], records: list[dict[str, str]]) -> None:
    ...
    for row in manifest_records:
        require(row.get("target_model") == "construction.contract.line", f"invalid target model: {row}")
        require(row.get("status") == "loadable", f"non-loadable manifest row: {row}")
        require(row.get("summary_amount_line") is True, f"missing summary flag: {row}")
...
def verify_validation_manifest(validation_manifest: dict[str, Any]) -> None:
    ...
    require(boundary.get("summary_amount_line") == "included", "summary amount line must be included")
    require(boundary.get("qty_contract") == "constant_1", "qty_contract policy drift")
    require(boundary.get("price_contract") == "source_total_amount", "price_contract policy drift")
    require(boundary.get("header_amount_write") == "excluded", "header amount write must be excluded")
    require(boundary.get("legacy_quantity_unit_price_detail") == "not_claimed", "legacy detail semantics must not be claimed")


def verify(asset_root: Path, lane: str) -> dict[str, Any]:
    asset_manifest = load_json(asset_root / "manifest" / "supplier_contract_line_asset_manifest_v1.json")
    external_manifest = load_json(asset_root / "manifest" / "supplier_contract_line_external_id_manifest_v1.json")
    validation_manifest = load_json(asset_root / "manifest" / "supplier_contract_line_validation_manifest_v1.json")
    records = parse_xml(asset_root / "20_business" / lane / "supplier_contract_summary_line_v1.xml")
    contract_refs = load_ref_set(asset_root / "manifest/supplier_contract_external_id_manifest_v1.json")

    verify_asset_manifest(asset_manifest, lane)
    verify_hashes(asset_root, asset_manifest)
    verify_xml_records(records, asset_manifest["counts"]["loadable_records"], contract_refs)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify supplier contract summary line XML migration assets.")
    parser.add_argument("--asset-root", default="migration_assets")
    parser.add_argument("--lane", default="supplier_contract_line")
    parser.add_argument("--check", action="store_true")
    args = parser.parse_args()

    try:
        result = verify(Path(args.asset_root), args.lane)
    except (SupplierContractLineAssetVerificationError, ET.ParseError, json.JSONDecodeError, InvalidOperation) as exc:
        payload = {"status": "FAIL", "error": str(exc), "db_writes": 0, "odoo_shell": False}
        print("SUPPLIER_CONTRACT_LINE_ASSET_VERIFY=" + json.dumps(payload, ensure_ascii=False, sort_keys=True))
        return 1 if args.check else 0
    print("SUPPLIER_CONTRACT_LINE_ASSET_VERIFY=" + json.dumps(result, ensure_ascii=False, sort_keys=True))
    return 0
