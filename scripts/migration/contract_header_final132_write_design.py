#!/usr/bin/env python3
"""Build a no-DB create-only design for the final 132-row contract header slice."""
...
REPO_ROOT = Path.cwd()
INPUT_CSV = REPO_ROOT / "artifacts/migration/contract_header_next_slice200_after_1200_v1.csv"
PRECHECK_JSON = REPO_ROOT / "artifacts/migration/contract_header_final132_readonly_precheck_result_v1.json"
OUTPUT_CSV = REPO_ROOT / "artifacts/migration/contract_header_final132_write_design_rows_v1.csv"
OUTPUT_JSON = REPO_ROOT / "artifacts/migration/contract_header_final132_write_design_result_v1.json"
EXPECTED_ROWS = 132
...
def main():
    ...
    if len(rows) != EXPECTED_ROWS:
        blockers.append({"reason": "unexpected_input_row_count", "expected": EXPECTED_ROWS, "actual": len(rows)})
    ...
    for index, row in enumerate(rows, start=1):
        ...
        design.update(
            {
                "write_design_id": f"contract_header_final132:{index:04d}",
    ...
    payload = {
        "status": "PASS" if len(design_rows) == EXPECTED_ROWS and not blockers else "PASS_WITH_BLOCKERS",
        "mode": "contract_header_final132_write_design_no_db",
        ...
        "next_step": "open no-DB authorization packet for the final 132-row design" if len(design_rows) == EXPECTED_ROWS and not blockers else "resolve design blockers",
    }
    write_json(OUTPUT_JSON, payload)
    print(
        "CONTRACT_HEADER_FINAL132_WRITE_DESIGN="
        + json.dumps(
            {
