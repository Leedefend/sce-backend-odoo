#!/usr/bin/env python3
"""Verify outflow request line XML migration assets."""
...
REQUIRED_FIELDS = {"request_id", "sequence", "legacy_line_id", "legacy_parent_id", "amount", "note"}
ALLOWED_XML_FIELDS = {
    "request_id",
    "sequence",
    "legacy_line_id",
...
class OutflowRequestLineAssetVerificationError(Exception):
    pass


def require(condition: bool, message: str) -> None:
    if not condition:
        raise OutflowRequestLineAssetVerificationError(message)
...
def load_ref_set(path: Path, legacy_key: str = "external_id") -> set[str]:
    manifest = load_json(path)
    return {
        str(row.get(legacy_key)).strip()
        for row in manifest.get("records", [])
        if str(row.get(legacy_key, "")).strip() and row.get("status") == "loadable"
    }
...
def verify_asset_manifest(asset_manifest: dict[str, Any], lane: str) -> None:
    ...
    require(asset_manifest.get("asset_package_id") == "outflow_request_line_sc_v1", "unexpected package id")
    ...
    require(asset_manifest.get("dependencies") == ["outflow_request_sc_v1", "supplier_contract_sc_v1"], "unexpected dependencies")
    ...
    require(asset_manifest.get("target", {}).get("model") == "payment.request.line", "unexpected target model")
...
def verify_xml_records(
    ...
    supplier_contract_refs: set[str],
) -> None:
    ...
    for row in records:
        require(row["id"].startswith("legacy_outflow_request_line_sc_"), f"invalid external id: {row['id']}")
        require(row["model"] == "payment.request.line", f"invalid XML model: {row['model']}")
        ...
        require(not forbidden, f"forbidden computed/default/related fields for {row['id']}: {forbidden}")
        require(row["request_id"] in request_refs, f"request ref does not resolve: {row['request_id']}")
        if row.get("contract_id"):
            require(row["contract_id"] in supplier_contract_refs, f"supplier contract ref does not resolve: {row['contract_id']}")
        require(positive_decimal(row["amount"]), f"amount must be positive for {row['id']}")
        require("line_fact=true" in row["note"], f"missing line_fact marker for {row['id']}")
        require("not_ledger=true" in row["note"], f"missing ledger boundary marker for {row['id']}")
...
def verify_external_manifest(external_manifest: dict[str, Any], records: list[dict[str, str]]) -> None:
    ...
    for row in manifest_records:
        require(row.get("target_model") == "payment.request.line", f"invalid target model: {row}")
        require(row.get("status") == "loadable", f"non-loadable manifest row: {row}")
        require(row.get("outflow_request_external_id"), f"missing parent request external id: {row}")
...
def verify_validation_manifest(validation_manifest: dict[str, Any]) -> None:
    ...
    require(boundary.get("line_fact") == "included", "line facts must be included")
    require(boundary.get("parent_outflow_request_anchor") == "required", "parent anchor policy drift")
    require(boundary.get("supplier_contract_anchor") == "optional_when_resolved", "supplier contract policy drift")
    require(boundary.get("ledger_fact") == "excluded", "ledger fact must be excluded")
    require(boundary.get("settlement_fact") == "excluded", "settlement fact must be excluded")


def verify(asset_root: Path, lane: str) -> dict[str, Any]:
    asset_manifest = load_json(asset_root / "manifest" / "outflow_request_line_asset_manifest_v1.json")
    external_manifest = load_json(asset_root / "manifest" / "outflow_request_line_external_id_manifest_v1.json")
    validation_manifest = load_json(asset_root / "manifest" / "outflow_request_line_validation_manifest_v1.json")
    records = parse_xml(asset_root / "20_business" / lane / "outflow_request_line_v1.xml")
    request_refs = load_ref_set(asset_root / "manifest/outflow_request_external_id_manifest_v1.json")
    supplier_contract_refs = load_ref_set(asset_root / "manifest/supplier_contract_external_id_manifest_v1.json")

    verify_asset_manifest(asset_manifest, lane)
    verify_hashes(asset_root, asset_manifest)
    verify_xml_records(records, asset_manifest["counts"]["loadable_records"], request_refs, supplier_contract_refs)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify outflow request line XML migration assets.")
    parser.add_argument("--asset-root", default="migration_assets")
    parser.add_argument("--lane", default="outflow_request_line")
    parser.add_argument("--check", action="store_true")
    args = parser.parse_args()

    try:
        result = verify(Path(args.asset_root), args.lane)
    except (OutflowRequestLineAssetVerificationError, ET.ParseError, json.JSONDecodeError, InvalidOperation) as exc:
        payload = {"status": "FAIL", "error": str(exc), "db_writes": 0, "odoo_shell": False}
        print("OUTFLOW_REQUEST_LINE_ASSET_VERIFY=" + json.dumps(payload, ensure_ascii=False, sort_keys=True))
        return 1 if args.check else 0
    print("OUTFLOW_REQUEST_LINE_ASSET_VERIFY=" + json.dumps(result, ensure_ascii=False, sort_keys=True))
    return 0
