#!/usr/bin/env python3
"""Replay targeted partner-master anchors required by direction-defer recovery into allowed DBs."""
...
def repo_root() -> Path:
    ...
    for candidate in candidates:
        if (candidate / "artifacts/migration/history_partner_master_direction_defer_replay_payload_v1.csv").exists():
            return candidate
    return Path.cwd()
...
INPUT_CSV = REPO_ROOT / "artifacts/migration/history_partner_master_direction_defer_replay_payload_v1.csv"
OUTPUT_JSON = ARTIFACT_ROOT / "history_partner_master_direction_defer_replay_write_result_v1.json"
ROLLBACK_CSV = ARTIFACT_ROOT / "history_partner_master_direction_defer_replay_rollback_targets_v1.csv"
EXPECTED_ROWS = 12
...
if duplicates:
    errors.append({"error": "duplicate_payload_identity", "samples": duplicates})
...
write_csv(ROLLBACK_CSV, ["id", "legacy_partner_source", "legacy_partner_id", "name"], created_rows)
result = {
    "status": "PASS" if len(created_rows) + len(skipped_rows) == EXPECTED_ROWS else "FAIL",
    "mode": "history_partner_master_direction_defer_replay_write",
