"""Readonly aggregate review for project_member neutral carrier writes."""
...
OUTPUT_JSON = Path("/mnt/artifacts/migration/project_member_neutral_aggregate_review_v1.json")
...
def main():
    ...
    records = model.search([("import_batch", "in", list(EXPECTED_BATCH_COUNTS))])
    missing_status_count = model.search_count([
        ("import_batch", "in", list(EXPECTED_BATCH_COUNTS)),
    ...
    payload = {
        ...
        "mode": "project_member_neutral_aggregate_review",
    ...
    print("PROJECT_MEMBER_NEUTRAL_AGGREGATE_REVIEW=" + json.dumps({
    ...
    if blocking_reasons:
        raise RuntimeError({"project_member_neutral_aggregate_review_failed": blocking_reasons})
