"""Bounded write for the next 500 duplicate-relation neutral project members."""
...
RUN_ID = os.environ.get("PROJECT_MEMBER_NEUTRAL_RUN_ID", "ITER-2026-04-14-1973N")
OUTPUT_TAG = os.environ.get("PROJECT_MEMBER_NEUTRAL_OUTPUT_TAG", "next500")
PRE_VISIBILITY_JSON = Path(f"/mnt/artifacts/migration/project_member_duplicate_relation_{OUTPUT_TAG}_pre_visibility_v1.json")
POST_VISIBILITY_JSON = Path(f"/mnt/artifacts/migration/project_member_duplicate_relation_{OUTPUT_TAG}_post_visibility_v1.json")
WRITE_RESULT_JSON = Path(f"/mnt/artifacts/migration/project_member_duplicate_relation_{OUTPUT_TAG}_write_result_v1.json")
ROLLBACK_TARGET_CSV = Path(f"/mnt/artifacts/migration/project_member_duplicate_relation_{OUTPUT_TAG}_rollback_targets_v1.csv")
...
def main():
    ...
    payload = {
        ...
        "mode": "project_member_duplicate_relation_neutral_next500_write",
    ...
    print("PROJECT_MEMBER_DUPLICATE_RELATION_NEUTRAL_NEXT500_WRITE=" + json.dumps({
    ...
    if post_errors:
        raise RuntimeError({"duplicate_relation_neutral_next500_write_failed": post_errors})
