#!/usr/bin/env python3
"""Replay legacy income invoice facts."""
...
def repo_root() -> Path:
    ...
    for candidate in candidates:
        if (candidate / "artifacts/migration/fresh_db_legacy_income_invoice_replay_adapter_result_v1.json").exists():
            return candidate
    return Path.cwd()
...
INPUT_MANIFEST = REPO_ROOT / "artifacts/migration/fresh_db_legacy_income_invoice_replay_adapter_result_v1.json"
INPUT_CSV = REPO_ROOT / "artifacts/migration/fresh_db_legacy_income_invoice_replay_payload_v1.csv"
OUTPUT_JSON = ARTIFACT_ROOT / "fresh_db_legacy_income_invoice_replay_write_result_v1.json"
...
bulk_load(INPUT_CSV, "tmp_legacy_income_invoice", COLUMNS)

env.cr.execute("SELECT COUNT(*) FROM sc_legacy_income_invoice_fact")  # noqa: F821
before = env.cr.fetchone()[0]  # noqa: F821

env.cr.execute(  # noqa: F821
    """
    INSERT INTO sc_legacy_income_invoice_fact (
      ...
      project_name, project_id, partner_legacy_id, partner_name, partner_tax_no,
      contract_legacy_id, contract_no, contract_name, invoice_no, invoice_code,
      invoice_type, invoice_content, tax_method, tax_type, tax_certificate_no,
      document_date, invoice_date,
      expected_receipt_date, created_time, creator_name, creator_legacy_user_id,
      qty, unit_price, amount_total, amount_no_tax, tax_amount, tax_rate,
      amount_contract, amount_received, bank_name, bank_account,
    ...
    SELECT
      ...
      NULLIF(t.tax_method, ''),
      NULLIF(t.tax_type, ''),
      NULLIF(t.tax_certificate_no, ''),
      NULLIF(t.document_date, '')::timestamp,
      NULLIF(t.invoice_date, '')::timestamp,
      NULLIF(t.expected_receipt_date, '')::timestamp,
      NULLIF(t.created_time, '')::timestamp,
      NULLIF(t.creator_name, ''),
      NULLIF(t.creator_legacy_user_id, ''),
      COALESCE(NULLIF(regexp_replace(t.qty, '[^0-9.-]', '', 'g'), '')::numeric, 0),
      COALESCE(NULLIF(regexp_replace(t.unit_price, '[^0-9.-]', '', 'g'), '')::numeric, 0),
      COALESCE(NULLIF(regexp_replace(t.amount_total, '[^0-9.-]', '', 'g'), '')::numeric, 0),
      COALESCE(NULLIF(regexp_replace(t.amount_no_tax, '[^0-9.-]', '', 'g'), '')::numeric, 0),
      COALESCE(NULLIF(regexp_replace(t.tax_amount, '[^0-9.-]', '', 'g'), '')::numeric, 0),
      COALESCE(NULLIF(regexp_replace(t.tax_rate, '[^0-9.-]', '', 'g'), '')::numeric, 0),
      COALESCE(NULLIF(regexp_replace(t.amount_contract, '[^0-9.-]', '', 'g'), '')::numeric, 0),
      COALESCE(NULLIF(regexp_replace(t.amount_received, '[^0-9.-]', '', 'g'), '')::numeric, 0),
      NULLIF(t.bank_name, ''),
      ...
      %s, NOW(), %s, NOW()
    FROM tmp_legacy_income_invoice t
    LEFT JOIN (
    ...
    ON CONFLICT (source_table, legacy_record_id) DO UPDATE SET
      ...
      partner_tax_no = EXCLUDED.partner_tax_no,
...
env.cr.execute("SELECT COUNT(*) FROM sc_legacy_income_invoice_fact WHERE active")  # noqa: F821
active_rows = env.cr.fetchone()[0]  # noqa: F821
env.cr.execute("SELECT COUNT(*) FROM sc_legacy_income_invoice_fact WHERE project_id IS NOT NULL")  # noqa: F821
project_linked = env.cr.fetchone()[0]  # noqa: F821
env.cr.execute("SELECT COALESCE(SUM(amount_total), 0), COALESCE(SUM(amount_no_tax), 0), COALESCE(SUM(tax_amount), 0), COALESCE(SUM(amount_received), 0) FROM sc_legacy_income_invoice_fact")  # noqa: F821
amount_total, amount_no_tax, tax_amount, amount_received = env.cr.fetchone()  # noqa: F821
env.cr.execute("SELECT source_table, COUNT(*) FROM sc_legacy_income_invoice_fact GROUP BY source_table ORDER BY source_table")  # noqa: F821
source_counts = dict(env.cr.fetchall())  # noqa: F821
env.cr.execute("SELECT fact_type, COUNT(*) FROM sc_legacy_income_invoice_fact GROUP BY fact_type ORDER BY fact_type")  # noqa: F821
fact_type_counts = dict(env.cr.fetchall())  # noqa: F821

payload = {
    "status": "PASS",
    "mode": "fresh_db_legacy_income_invoice_replay_write",
    ...
    "amount_received_sum": str(amount_received),
    "source_counts": source_counts,
    "fact_type_counts": fact_type_counts,
    "db_writes": max(after - before, 0),
    "decision": "legacy_income_invoice_replay_complete",
}
write_json(OUTPUT_JSON, payload)
print("FRESH_DB_LEGACY_INCOME_INVOICE_REPLAY_WRITE=" + json.dumps(payload, ensure_ascii=False, sort_keys=True))
