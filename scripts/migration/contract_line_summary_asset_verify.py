#!/usr/bin/env python3
"""Verify repository-tracked contract summary-line XML migration assets."""
...
FORBIDDEN_XML_FIELDS = {"boq_line_id", "boq_code", "boq_name", "wbs_id", "uom_id"}
FORBIDDEN_ASSET_TOKENS = ("payment", "settlement", "account", "security", "record_rule", "__manifest__")


class ContractLineAssetVerificationError(Exception):
    pass


def require(condition: bool, message: str) -> None:
    if not condition:
        raise ContractLineAssetVerificationError(message)
...
def verify_asset_manifest(asset_manifest: dict[str, Any], lane: str) -> None:
    ...
    require(asset_manifest.get("asset_package_id") == "contract_line_sc_v1", "unexpected contract-line package id")
    ...
    require(asset_manifest.get("dependencies") == ["contract_sc_v1"], "unexpected dependencies")
    ...
    require(asset_manifest.get("target", {}).get("semantic") == "summary_line_only", "unexpected target semantic")
    for asset in asset_manifest.get("assets", []):
...
def verify_xml_records(records: list[dict[str, str]], expected_count: int, contract_refs: set[str]) -> None:
    ...
    contracts = [row.get("contract_id", "") for row in records]
    require(len(ids) == len(set(ids)), "duplicate XML external ids")
    require(len(contracts) == len(set(contracts)), "more than one summary line per contract")
    for row in records:
        require(row["id"].startswith("legacy_contract_line_summary_sc_"), f"invalid external id: {row['id']}")
        ...
        require(not forbidden, f"BOQ detail fields must not be written for {row['id']}: {forbidden}")
        require(row["contract_id"] in contract_refs, f"contract ref does not resolve: {row['contract_id']}")
        require(row["qty_contract"] == "1.0", f"summary qty must be 1.0 for {row['id']}")
        require(positive_decimal(row["price_contract"]), f"price_contract must be positive for {row['id']}")
        require("not a reconstructed BOQ detail" in row["note"], f"missing summary boundary note for {row['id']}")
...
def verify_external_manifest(external_manifest: dict[str, Any], records: list[dict[str, str]]) -> None:
    ...
    for row in manifest_records:
        ...
        require(row.get("target_semantic") == "summary_line_only", f"invalid target semantic: {row}")
...
def verify_validation_manifest(validation_manifest: dict[str, Any]) -> None:
    ...
    required = {"contract_external_id_resolves", "amount_positive", "one_summary_line_per_contract", "boq_detail_not_fabricated"}
    ...
    require(boundary.get("boq_detail_reconstruction") == "excluded", "BOQ detail reconstruction must be excluded")
...
def verify(asset_root: Path, lane: str) -> dict[str, Any]:
    ...
    records = parse_xml(asset_root / "20_business" / lane / f"{lane}_summary_v1.xml")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify contract summary-line XML migration assets.")
    parser.add_argument("--asset-root", default="migration_assets")
    parser.add_argument("--lane", default="contract_line")
    parser.add_argument("--check", action="store_true")
    args = parser.parse_args()

    try:
        result = verify(Path(args.asset_root), args.lane)
    except (ContractLineAssetVerificationError, ET.ParseError, json.JSONDecodeError) as exc:
        payload = {"status": "FAIL", "error": str(exc), "db_writes": 0, "odoo_shell": False}
        print("CONTRACT_LINE_SUMMARY_ASSET_VERIFY=" + json.dumps(payload, ensure_ascii=False, sort_keys=True))
        return 1 if args.check else 0
    print("CONTRACT_LINE_SUMMARY_ASSET_VERIFY=" + json.dumps(result, ensure_ascii=False, sort_keys=True))
    return 0
