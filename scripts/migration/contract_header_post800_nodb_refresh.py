...
RESULT_JSON = ARTIFACT_DIR / "contract_header_post800_nodb_refresh_result_v1.json"
REMAINING_CSV = ARTIFACT_DIR / "contract_header_remaining_after_800_v1.csv"
NEXT_SLICE_CSV = ARTIFACT_DIR / "contract_header_next_slice200_after_800_v1.csv"

DATABASE = "sc_demo"
EXPECTED_INPUT_ROWS = 1332
EXPECTED_WRITTEN_ROWS = 800
EXPECTED_REMAINING_ROWS = 532
...
if len(written_existing_ids) != EXPECTED_WRITTEN_ROWS:
    errors.append({"kind": "existing_written_count_not_800", "actual": len(written_existing_ids)})
...
if len(remaining_rows) != EXPECTED_REMAINING_ROWS:
    errors.append({"kind": "remaining_candidate_count_not_532", "actual": len(remaining_rows)})
...
result = {
    ...
    "mode": "contract_header_post800_nodb_refresh",
...
if errors:
    raise Exception("CONTRACT_HEADER_POST800_NODB_REFRESH_FAILED=" + json.dumps(result, ensure_ascii=False, sort_keys=True))

print("CONTRACT_HEADER_POST800_NODB_REFRESH=" + json.dumps({
