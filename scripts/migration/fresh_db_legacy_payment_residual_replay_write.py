...
def bulk_load(csv_path: Path, temp_table: str, columns: list[str]) -> None:
    ...
    if unknown_columns:
        raise RuntimeError({"unexpected_legacy_payment_residual_payload_columns": unknown_columns})
    ...
    with csv_path.open("r", encoding="utf-8-sig", newline="") as handle:
        env.cr.copy_expert(  # noqa: F821
            f"COPY {temp_table} ({', '.join(header)}) FROM STDIN WITH CSV HEADER",
            handle,
        )
...
env.cr.execute(  # noqa: F821
    ...
    INSERT INTO sc_legacy_payment_residual_fact (
      ...
      handler_phone, regional_tax_contact, regional_tax_contact_phone,
      operation_address, tax_report_management_no, taxpayer_name,
      taxpayer_identifier, contract_name, contract_start_date,
      contract_end_date, counterparty_tax_identifier,
    ...
    SELECT
      ...
      NULLIF(t.handler_phone, ''),
      NULLIF(t.regional_tax_contact, ''),
      NULLIF(t.regional_tax_contact_phone, ''),
      NULLIF(t.operation_address, ''),
      NULLIF(t.tax_report_management_no, ''),
      NULLIF(t.taxpayer_name, ''),
      NULLIF(t.taxpayer_identifier, ''),
      NULLIF(t.contract_name, ''),
      NULLIF(t.contract_start_date, '')::date,
      NULLIF(t.contract_end_date, '')::date,
      NULLIF(t.counterparty_tax_identifier, ''),
      NULLIF(t.note, ''),
    ...
    ON CONFLICT (source_table, legacy_record_id) DO UPDATE SET
      ...
      handler_phone = EXCLUDED.handler_phone,
      regional_tax_contact = EXCLUDED.regional_tax_contact,
      regional_tax_contact_phone = EXCLUDED.regional_tax_contact_phone,
      operation_address = EXCLUDED.operation_address,
      tax_report_management_no = EXCLUDED.tax_report_management_no,
      taxpayer_name = EXCLUDED.taxpayer_name,
      taxpayer_identifier = EXCLUDED.taxpayer_identifier,
      contract_name = EXCLUDED.contract_name,
      contract_start_date = EXCLUDED.contract_start_date,
      contract_end_date = EXCLUDED.contract_end_date,
      counterparty_tax_identifier = EXCLUDED.counterparty_tax_identifier,
      note = EXCLUDED.note,
