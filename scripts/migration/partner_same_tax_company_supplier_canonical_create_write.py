"""Create partner records for same-tax company/supplier canonical rows."""
...
PRE_WRITE_SNAPSHOT_CSV = Path("/mnt/artifacts/migration/partner_l4_same_tax_company_supplier_canonical_102_pre_write_snapshot_v1.csv")
POST_WRITE_SNAPSHOT_CSV = Path("/mnt/artifacts/migration/partner_l4_same_tax_company_supplier_canonical_102_post_write_snapshot_v1.csv")
...
EXPECTED_COUNT = 102
RUN_ID = "ITER-2026-04-14-PARTNER-L4-SAME-TAX-COMPANY-SUPPLIER-CANONICAL-102-WRITE"
...
def build_create_vals(row):
    ...
    return {
        ...
        "legacy_source_evidence": "partner_l4_same_tax_company_supplier_canonical_write_design_v1",
    }
...
def main():
    ...
    for index, row in enumerate(rows, start=2):
        row_errors = []
        if clean(row.get("proposed_action")) != "create_company_supplier_partner_with_canonical_tax_number":
        ...
        if not clean(row.get("legacy_credit_code")):
            row_errors.append("missing_legacy_credit_code")
        if row_errors:
        ...
        precheck_errors.append(
            {
                ...
                "matches": [
                    {"id": rec.id, "legacy_partner_id": rec.legacy_partner_id or "", "name": rec.name or ""}
                    for rec in existing
                ],
            }
        )
    ...
    payload = {
        ...
        "mode": "partner_same_tax_company_supplier_canonical_create_write",
    ...
    print("PARTNER_SAME_TAX_COMPANY_SUPPLIER_CANONICAL_WRITE_RESULT=" + json.dumps({"status": payload["status"], "created": payload["created"], "updated": payload["updated"], "errors": payload["errors"], "rollback_target_rows": payload["rollback_target_rows"]}, ensure_ascii=False, sort_keys=True))
    if post_errors:
        raise RuntimeError({"partner_same_tax_company_supplier_canonical_write_failed": post_errors})
