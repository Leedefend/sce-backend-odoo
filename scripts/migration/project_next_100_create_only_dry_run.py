"""Build and dry-run the next non-overlapping 100-row project candidate."""
...
PROJECT_30_POST_WRITE_CSV = Path("artifacts/migration/project_create_only_post_write_snapshot_v1.csv")
PROJECT_100_POST_WRITE_CSV = Path("artifacts/migration/project_create_only_expand_post_write_snapshot_v1.csv")
OUTPUT_CSV = Path("artifacts/migration/project_next_100_candidate_v2.csv")
OUTPUT_JSON = Path("artifacts/migration/project_next_100_dry_run_result_v2.json")
...
def existing_project_ids():
    ids = set()
    for path in (PROJECT_30_POST_WRITE_CSV, PROJECT_100_POST_WRITE_CSV):
...
payload = {
    ...
    "mode": "project_next_100_create_only_dry_run_no_db_no_orm",
    ...
    "errors": errors,
    "results": results,
    "next_step": "open write authorization packet for this 100-row project create-only candidate",
}
write_json(OUTPUT_JSON, payload)
print(
    "PROJECT_NEXT_100_DRY_RUN="
    + json.dumps(
        {
