"""Authorized remaining project 25-row create-only write for Odoo shell."""
...
INPUT_CSV = Path("/mnt/artifacts/migration/project_remaining_candidate_v6.csv")
PACKET_JSON = Path("/mnt/artifacts/migration/project_remaining_25_write_authorization_packet.json")
PRE_WRITE_SNAPSHOT_CSV = Path("/mnt/artifacts/migration/project_remaining_25_pre_write_snapshot.csv")
...
def main():
    ...
    if packet.get("status") != "PASS" or packet.get("payload_rows") != EXPECTED_COUNT:
        raise RuntimeError({"authorization_packet_invalid": packet})
    ...
    if len(post_records) != EXPECTED_COUNT:
        post_errors.append("post_write_identity_count_not_25")
    ...
    payload = {
        ...
        "mode": "project_remaining_25_create_only_write",
        ...
        "artifacts": {"pre_write_snapshot": str(PRE_WRITE_SNAPSHOT_CSV), "post_write_snapshot": str(POST_WRITE_SNAPSHOT_CSV)},
    }
    write_json(WRITE_RESULT_JSON, payload)
    print("PROJECT_REMAINING_25_WRITE_RESULT=" + json.dumps(payload["summary"] | {"status": payload["status"]}, ensure_ascii=False, sort_keys=True))
    if post_errors:
        raise RuntimeError({"project_remaining_25_write_post_errors": post_errors})
