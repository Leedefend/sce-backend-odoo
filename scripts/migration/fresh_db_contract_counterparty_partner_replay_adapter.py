#!/usr/bin/env python3
"""Build replay payload for contract counterparty partner supplemental anchors."""
...
REPO_ROOT = Path.cwd()
INPUT_XML = REPO_ROOT / "migration_assets/10_master/contract_counterparty_partner/contract_counterparty_partner_master_v1.xml"
INPUT_MANIFEST = REPO_ROOT / "migration_assets/manifest/contract_counterparty_partner_asset_manifest_v1.json"
OUTPUT_JSON = REPO_ROOT / "artifacts/migration/fresh_db_contract_counterparty_partner_replay_adapter_result_v1.json"
OUTPUT_CSV = REPO_ROOT / "artifacts/migration/fresh_db_contract_counterparty_partner_replay_payload_v1.csv"
...
payload = {
    ...
    "decision": "contract_counterparty_partner_replay_payload_ready" if status == "PASS" else "STOP_REVIEW_REQUIRED",
    "next_step": "write contract counterparty supplemental anchors into allowed replay db",
}
write_csv(
    OUTPUT_CSV,
    [
...
print(
    "FRESH_DB_CONTRACT_COUNTERPARTY_PARTNER_REPLAY_ADAPTER="
    + json.dumps(
        {
            "status": status,
            "replay_payload_rows": len(payload_rows),
