#!/usr/bin/env python3
"""Mirror old SCBS deduction paid registration list into action 897."""
...
PATH = Path("/tmp/scbs_55_old_live_full_rows_seq033_deduction_paid.json.gz")
SURFACE = "online_old_scbs:T_KK_SJDJB:list897"
ACTION_ID = 897
SOURCE_TABLE = "T_KK_SJDJB"
...
def row_key(row: dict[str, Any], index: int) -> str:
    ...
    child = clean(row.get("Id$T_KK_SJDJB_CB") or row.get("RowIndex") or index)
...
def visible_payload(row: dict[str, Any]) -> dict[str, str]:
    return {
        ...
        "标题": clean(row.get("BTMC")),
        "本次实缴数": amount_text(row.get("BCSJS$T_KK_SJDJB_CB")),
        "是否退回": clean(row.get("SFTH$T_KK_SJDJB_CB")),
        "上缴内容": clean(row.get("SJNR$T_KK_SJDJB_CB")),
        "本次计划已缴数": amount_text(row.get("BCJHYJS$T_KK_SJDJB_CB")),
...
for index, row in enumerate(rows, start=1):
    ...
    vals = {
        ...
        "expense_type": "扣款实缴登记",
        "summary": clean(row.get("BTMC") or row.get("SJNR$T_KK_SJDJB_CB")),
        "amount": amount(row.get("BCSJS$T_KK_SJDJB_CB")),
        "approved_amount": amount(row.get("BCSJS$T_KK_SJDJB_CB")),
        ...
        "legacy_visible_title": clean(row.get("BTMC")),
        "legacy_visible_adjustment_item": clean(row.get("SJNR$T_KK_SJDJB_CB")),
        "legacy_visible_amount": amount_text(row.get("BCSJS$T_KK_SJDJB_CB")),
        "legacy_visible_returned_flag": clean(row.get("SFTH$T_KK_SJDJB_CB")),
        "legacy_visible_note": clean(row.get("BZ$T_KK_SJDJB_CB")),
        ...
        % (SURFACE, clean(row.get("Id")), clean(row.get("Id$T_KK_SJDJB_CB")), clean(row.get("FJ") or row.get("f_FJ"))),
        "active": True,
    }
    if rec:
        rec.write(vals)
