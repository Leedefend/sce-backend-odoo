#!/usr/bin/env python3
"""Verify repository-tracked actual outflow XML migration assets."""
...
ALLOWED_XML_FIELDS = {"type", "project_id", "partner_id", "amount", "date_request", "note"}
...
class ActualOutflowAssetVerificationError(Exception):
    pass


def require(condition: bool, message: str) -> None:
    if not condition:
        raise ActualOutflowAssetVerificationError(message)
...
def verify_asset_manifest(asset_manifest: dict[str, Any], lane: str) -> None:
    ...
    require(asset_manifest.get("asset_package_id") == "actual_outflow_sc_v1", "unexpected actual outflow package id")
...
def verify_xml_records(
    ...
    for row in records:
        require(row["id"].startswith("legacy_actual_outflow_sc_"), f"invalid external id: {row['id']}")
        ...
        require(not unexpected, f"unexpected actual outflow fields for {row['id']}: {unexpected}")
        forbidden = sorted(FORBIDDEN_XML_FIELDS & set(row))
        require(not forbidden, f"forbidden actual outflow fields for {row['id']}: {forbidden}")
        ...
        require("[migration:actual_outflow_core]" in row["note"], f"missing migration note marker for {row['id']}")
...
def verify_validation_manifest(validation_manifest: dict[str, Any]) -> None:
    ...
    require(boundary.get("carrier") == "draft_carrier", "carrier must be draft_carrier")
...
def verify(asset_root: Path, lane: str) -> dict[str, Any]:
    asset_manifest = load_json(asset_root / "manifest" / "actual_outflow_asset_manifest_v1.json")
    external_manifest = load_json(asset_root / "manifest" / "actual_outflow_external_id_manifest_v1.json")
    validation_manifest = load_json(asset_root / "manifest" / "actual_outflow_validation_manifest_v1.json")
    records = parse_xml(asset_root / "20_business" / lane / "actual_outflow_core_v1.xml")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify actual outflow XML migration assets.")
    parser.add_argument("--asset-root", default="migration_assets")
    parser.add_argument("--lane", default="actual_outflow")
    parser.add_argument("--check", action="store_true")
    args = parser.parse_args()

    try:
        result = verify(Path(args.asset_root), args.lane)
    except (ActualOutflowAssetVerificationError, ET.ParseError, json.JSONDecodeError) as exc:
        payload = {"status": "FAIL", "error": str(exc), "db_writes": 0, "odoo_shell": False}
        print("ACTUAL_OUTFLOW_ASSET_VERIFY=" + json.dumps(payload, ensure_ascii=False, sort_keys=True))
        return 1 if args.check else 0
    print("ACTUAL_OUTFLOW_ASSET_VERIFY=" + json.dumps(result, ensure_ascii=False, sort_keys=True))
    return 0
