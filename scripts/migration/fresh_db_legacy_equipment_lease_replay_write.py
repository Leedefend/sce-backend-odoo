#!/usr/bin/env python3
"""Replay legacy equipment and lease facts."""
...
def repo_root() -> Path:
    ...
    for candidate in candidates:
        if (candidate / "artifacts/migration/fresh_db_legacy_equipment_lease_replay_adapter_result_v1.json").exists():
            return candidate
    return Path.cwd()
...
INPUT_MANIFEST = REPO_ROOT / "artifacts/migration/fresh_db_legacy_equipment_lease_replay_adapter_result_v1.json"
INPUT_CSV = REPO_ROOT / "artifacts/migration/fresh_db_legacy_equipment_lease_replay_payload_v1.csv"
OUTPUT_JSON = ARTIFACT_ROOT / "fresh_db_legacy_equipment_lease_replay_write_result_v1.json"
...
bulk_load(INPUT_CSV, "tmp_legacy_equipment_lease", COLUMNS)

env.cr.execute("SELECT COUNT(*) FROM sc_legacy_equipment_lease_fact")  # noqa: F821
before = env.cr.fetchone()[0]  # noqa: F821

env.cr.execute(  # noqa: F821
    """
    INSERT INTO sc_legacy_equipment_lease_fact (
      ...
      contract_legacy_id, contract_no, equipment_legacy_id, equipment_name,
      equipment_spec, equipment_uom, work_part, department_name,
      document_date, start_date, end_date, created_time, creator_name,
      creator_legacy_user_id, qty, unit_price, amount_total, amount_no_tax,
      tax_amount, tax_rate, amount_payable, amount_deduction, bank_name,
    ...
    SELECT
      ...
      NULLIF(t.equipment_legacy_id, ''),
      NULLIF(t.equipment_name, ''),
      NULLIF(t.equipment_spec, ''),
      NULLIF(t.equipment_uom, ''),
      NULLIF(t.work_part, ''),
      ...
      %s, NOW(), %s, NOW()
    FROM tmp_legacy_equipment_lease t
    LEFT JOIN (
    ...
    ON CONFLICT (source_table, legacy_record_id) DO UPDATE SET
      ...
      equipment_legacy_id = EXCLUDED.equipment_legacy_id,
      equipment_name = EXCLUDED.equipment_name,
      equipment_spec = EXCLUDED.equipment_spec,
      equipment_uom = EXCLUDED.equipment_uom,
...
env.cr.execute("SELECT COUNT(*) FROM sc_legacy_equipment_lease_fact WHERE active")  # noqa: F821
active_rows = env.cr.fetchone()[0]  # noqa: F821
env.cr.execute("SELECT COUNT(*) FROM sc_legacy_equipment_lease_fact WHERE project_id IS NOT NULL")  # noqa: F821
project_linked = env.cr.fetchone()[0]  # noqa: F821
env.cr.execute("SELECT COALESCE(SUM(amount_total), 0), COALESCE(SUM(amount_payable), 0), COALESCE(SUM(amount_deduction), 0) FROM sc_legacy_equipment_lease_fact")  # noqa: F821
amount_total, amount_payable, amount_deduction = env.cr.fetchone()  # noqa: F821
env.cr.execute("SELECT source_table, COUNT(*) FROM sc_legacy_equipment_lease_fact GROUP BY source_table ORDER BY source_table")  # noqa: F821
source_counts = dict(env.cr.fetchall())  # noqa: F821
env.cr.execute("SELECT fact_type, COUNT(*) FROM sc_legacy_equipment_lease_fact GROUP BY fact_type ORDER BY fact_type")  # noqa: F821
fact_type_counts = dict(env.cr.fetchall())  # noqa: F821

payload = {
    "status": "PASS",
    "mode": "fresh_db_legacy_equipment_lease_replay_write",
    ...
    "amount_deduction_sum": str(amount_deduction),
    "source_counts": source_counts,
    "fact_type_counts": fact_type_counts,
    "db_writes": max(after - before, 0),
    "decision": "legacy_equipment_lease_replay_complete",
}
write_json(OUTPUT_JSON, payload)
print("FRESH_DB_LEGACY_EQUIPMENT_LEASE_REPLAY_WRITE=" + json.dumps(payload, ensure_ascii=False, sort_keys=True))
