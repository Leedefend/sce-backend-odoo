#!/usr/bin/env python3
"""Build replay payload for legacy fund daily snapshot facts."""
...
INPUT_XML = REPO_ROOT / "migration_assets/30_relation/legacy_fund_daily_snapshot/legacy_fund_daily_snapshot_v1.xml"
OUTPUT_CSV = REPO_ROOT / "artifacts/migration/fresh_db_legacy_fund_daily_snapshot_replay_payload_v1.csv"
OUTPUT_JSON = REPO_ROOT / "artifacts/migration/fresh_db_legacy_fund_daily_snapshot_replay_adapter_result_v1.json"
...
def main() -> int:
    ...
    for _, elem in ET.iterparse(INPUT_XML, events=("end",)):
        if elem.tag != "record":
            continue
        if clean(elem.attrib.get("model")) != "sc.legacy.fund.daily.snapshot.fact":
            elem.clear()
            continue
