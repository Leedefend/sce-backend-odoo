#!/usr/bin/env python3
"""Verify legacy invoice and tax XML migration assets."""
...
REQUIRED_FIELDS = {
    "legacy_source_table", "legacy_record_id", "source_family", "direction",
    "project_id", "legacy_project_id", "source_amount_field", "import_batch",
}
ALLOWED_FIELDS = {
...
FORBIDDEN_FIELDS = {
    ...
    "payment_request_id", "validation_status", "review_ids", "res_id",
}
ALLOWED_FAMILIES = {"input_invoice_handover", "output_invoice_register", "invoice_issue_request", "prepaid_tax_register"}
ALLOWED_DIRECTIONS = {"input_invoice", "output_invoice", "prepaid_tax"}


class InvoiceTaxAssetVerificationError(Exception):
    pass


def require(condition: bool, message: str) -> None:
    if not condition:
        raise InvoiceTaxAssetVerificationError(message)
...
def verify(asset_root: Path, lane: str) -> dict[str, Any]:
    asset_manifest = load_json(asset_root / "manifest/legacy_invoice_tax_asset_manifest_v1.json")
    external_manifest = load_json(asset_root / "manifest/legacy_invoice_tax_external_id_manifest_v1.json")
    validation_manifest = load_json(asset_root / "manifest/legacy_invoice_tax_validation_manifest_v1.json")
    records = parse_xml(asset_root / "30_relation" / lane / "legacy_invoice_tax_v1.xml")
    require(asset_manifest.get("asset_package_id") == "legacy_invoice_tax_sc_v1", "unexpected package id")
    ...
    for row in records:
        require(row["id"].startswith("legacy_invoice_tax_sc_"), f"invalid external id: {row['id']}")
        require(row["model"] == "sc.legacy.invoice.tax.fact", f"invalid model: {row['model']}")
    ...
    require(boundary.get("tax_ledger") == "excluded", "tax_ledger must be excluded")
    require(boundary.get("payment_state") == "excluded", "payment_state must be excluded")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify legacy invoice/tax XML migration assets.")
    parser.add_argument("--asset-root", default="migration_assets")
    parser.add_argument("--lane", default="legacy_invoice_tax")
    parser.add_argument("--check", action="store_true")
    args = parser.parse_args()
    try:
        result = verify(Path(args.asset_root), args.lane)
    except (InvoiceTaxAssetVerificationError, ET.ParseError, json.JSONDecodeError) as exc:
        print("LEGACY_INVOICE_TAX_ASSET_VERIFY=" + json.dumps({"status": "FAIL", "error": str(exc), "db_writes": 0, "odoo_shell": False}, ensure_ascii=False, sort_keys=True))
        return 1 if args.check else 0
    print("LEGACY_INVOICE_TAX_ASSET_VERIFY=" + json.dumps(result, ensure_ascii=False, sort_keys=True))
    return 0
