"""Readonly post-write review for missing-tax single-source partner write."""
...
OUTPUT_JSON = Path("/mnt/artifacts/migration/partner_l4_missing_tax_single_source_1554_post_write_review_result_v1.json")
EXPECTED_COUNT = 1554
...
def main():
        ...
        rows.append(
            {
                ...
                "customer_rank": rec.customer_rank if rec else "",
                "supplier_rank": rec.supplier_rank if rec else "",
                "match_count": len(matches),
                "in_design": legacy_id in design_id_set,
                "rollback_eligible": bool(rec)
                and legacy_source == LEGACY_SOURCE
                and legacy_id in design_id_set
                and len(matches) == 1,
            }
        )
    ...
    payload = {
        ...
        "mode": "partner_missing_tax_single_source_post_write_readonly_review",
    ...
    print("PARTNER_MISSING_TAX_SINGLE_SOURCE_POST_WRITE_REVIEW=" + json.dumps({
    ...
    if blocking_reasons:
        raise RuntimeError({"partner_missing_tax_single_source_post_write_review_failed": blocking_reasons})
