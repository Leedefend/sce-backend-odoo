#!/usr/bin/env python3
"""Verify repository-tracked receipt core XML migration assets."""
...
class ReceiptAssetVerificationError(Exception):
    pass


def require(condition: bool, message: str) -> None:
    if not condition:
        raise ReceiptAssetVerificationError(message)
...
def verify_asset_manifest(asset_manifest: dict[str, Any], lane: str) -> None:
    ...
    require(asset_manifest.get("asset_package_id") == "receipt_sc_v1", "unexpected receipt package id")
    ...
    require(
        asset_manifest.get("dependencies") == ["project_sc_v1", "partner_sc_v1", "receipt_counterparty_partner_sc_v1", "contract_sc_v1"],
    ...
    require(asset_manifest.get("target", {}).get("type") == "receive", "unexpected target type")
    for asset in asset_manifest.get("assets", []):
...
def verify_xml_records(
    ...
    for row in records:
        require(row["id"].startswith("legacy_receipt_sc_"), f"invalid external id: {row['id']}")
        ...
        require(not forbidden, f"forbidden receipt fields for {row['id']}: {forbidden}")
        require(row["type"] == "receive", f"type must be receive for {row['id']}")
        ...
        require("[migration:receipt_core]" in row["note"], f"missing migration note marker for {row['id']}")
...
def verify_external_manifest(external_manifest: dict[str, Any], records: list[dict[str, str]]) -> None:
    ...
    for row in manifest_records:
        ...
        require(row.get("target_type") == "receive", f"invalid target type: {row}")
...
def verify(asset_root: Path, lane: str) -> dict[str, Any]:
    ...
    records = parse_xml(asset_root / "20_business" / lane / f"{lane}_core_v1.xml")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify receipt core XML migration assets.")
    parser.add_argument("--asset-root", default="migration_assets")
    parser.add_argument("--lane", default="receipt")
    parser.add_argument("--check", action="store_true")
    args = parser.parse_args()

    try:
        result = verify(Path(args.asset_root), args.lane)
    except (ReceiptAssetVerificationError, ET.ParseError, json.JSONDecodeError) as exc:
        payload = {"status": "FAIL", "error": str(exc), "db_writes": 0, "odoo_shell": False}
        print("RECEIPT_CORE_ASSET_VERIFY=" + json.dumps(payload, ensure_ascii=False, sort_keys=True))
        return 1 if args.check else 0
    print("RECEIPT_CORE_ASSET_VERIFY=" + json.dumps(result, ensure_ascii=False, sort_keys=True))
    return 0
