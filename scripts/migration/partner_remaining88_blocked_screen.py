...
OUTPUT_JSON = Path("artifacts/migration/partner_l4_remaining88_blocked_screen_result_v1.json")
OUTPUT_CSV = Path("artifacts/migration/partner_l4_remaining88_blocked_screen_rows_v1.csv")
SAMPLE_CSV = Path("artifacts/migration/partner_l4_remaining88_blocked_screen_samples_v1.csv")
EXPECTED_BLOCKED_ROWS = 88
...
def classify(row: dict[str, str]) -> str:
    ...
    if "same_tax_multiple_names" in blockers and clean(row.get("legacy_partner_source")) == "company_supplier":
        return "remaining_same_tax_company_supplier_conflict_screen"
    if "same_tax_multiple_names" in blockers:
        return "remaining_same_tax_company_duplicate_conflict_screen"
    if "missing_tax_number" in blockers and clean(row.get("legacy_partner_source")) == "supplier":
        return "supplier_missing_tax_single_screen"
    if "deduplicated_group_not_create_only" in blockers and clean(row.get("legacy_partner_source")) == "company_supplier":
        return "company_supplier_duplicate_group_screen"
...
def main() -> int:
    ...
    for row in blocked_rows:
        route = classify(row)
        combo = ";".join(sorted(blocker_set(row)))
    ...
    result = {
        ...
        "mode": "partner_l4_remaining88_blocked_screen",
        "input": str(INPUT_CSV),
        ...
        "next_gate": "Open the largest remaining duplicate-group route; no DB write is authorized.",
    }
    OUTPUT_JSON.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT_JSON.write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print("PARTNER_REMAINING88_BLOCKED_SCREEN=" + json.dumps({
