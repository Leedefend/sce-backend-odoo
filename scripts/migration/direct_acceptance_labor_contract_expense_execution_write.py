# -*- coding: utf-8 -*-
"""Project accepted direct-project labor contracts into formal expense execution."""
...
ACCEPTANCE_LABEL = "劳务合同"
TARGET_MODEL = "construction.contract"
TARGET_WRAPPER_MODEL = "construction.contract.expense"
LEGACY_ID_PREFIX = "direct_acceptance:labor_contract:"
MIGRATION_MARKER = "[migration:direct_acceptance_labor_contract_to_expense_execution]"
...
OUTPUT_JSON = artifact_root() / "direct_acceptance_labor_contract_expense_execution_write_result_v1.json"
...
def resolve_project(fact, payload: dict):
    ...
    project_name = (
        text(fact.legacy_visible_03)
        or text(fact.legacy_visible_20)
        or text(fact.project_name)
        or first_text(payload, "f_GCMC", "TSXMMC", "XMMC", "ProjectName")
    )
    if not project_name:
        project_name = "直营项目劳务合同未匹配项目"
...
def resolve_partner(fact, payload: dict):
    ...
    name = text(fact.legacy_visible_06) or first_text(payload, "f_BZZ", "SGDWMC", "GYDW") or text(fact.partner_name)
    if not name:
        name = "直营项目劳务合同未匹配劳务单位"
    ...
    if "legacy_partner_id" in Partner._fields:
        vals["legacy_partner_id"] = "direct_acceptance_labor:" + name
    if "legacy_partner_source" in Partner._fields:
        vals["legacy_partner_source"] = "direct_acceptance_labor_contract"
...
def labor_category():
    category = env.ref("smart_construction_core.dict_expense_contract_category_labor", raise_if_not_found=False)  # noqa: F821
    if category:
        return category
    return env["sc.dictionary"].sudo().search(  # noqa: F821
        [("type", "=", "expense_contract_category"), ("code", "=", "labor")],
        limit=1,
    )


def execution_title(title: str) -> str:
    # The formal execution action excludes titles containing 结算. Keep the
    # accepted title in the note's source_title evidence and route visible
    # execution title fields through a non-filtered wording.
    return title.replace("结算", "办理") if "结算" in title else title


def attachment_display_and_ref(fact, payload: dict) -> tuple[str, str]:
    visible_attachment = text(fact.legacy_visible_17)
...
def contract_values(fact, category):
    ...
    visible_project_name = text(fact.legacy_visible_03)
    visible_contract_date = text(fact.legacy_visible_04)
    visible_title = text(fact.legacy_visible_05)
    visible_labor_unit = text(fact.legacy_visible_06)
    visible_team_leader = text(fact.legacy_visible_07)
    visible_amount = text(fact.legacy_visible_08)
    visible_settlement_ratio = text(fact.legacy_visible_09)
    ...
    visible_pricing_method = text(fact.legacy_visible_14)
    visible_work_part = text(fact.legacy_visible_15)
    visible_contract_no = text(fact.legacy_visible_16)
    visible_attachment, attachment_ref = attachment_display_and_ref(fact, payload)
    visible_creator = text(fact.legacy_visible_18) or text(fact.creator_name)
    visible_payment_terms = text(fact.legacy_visible_19)
    visible_push_project_name = text(fact.legacy_visible_20)
    visible_creator = visible_creator or first_text(payload, "f_LRR", "LRR", "XGR")
    created_time = parse_datetime(fact.created_time) or parse_datetime(first_text(payload, "LRSJ", "f_LRSJ", "XGSJ"))
    attachment_text = visible_attachment
    if attachment_ref:
        attachment_text = (attachment_text + "\n" if attachment_text else "") + "legacy-file-id://" + attachment_ref
    amount_source_text = visible_amount or first_text(payload, "ZHSJE", "ZJE", "JE")
    ...
    accepted_title = visible_title or visible_document_no or text(fact.document_title) or text(fact.legacy_record_id)
    content_text = visible_payment_terms or visible_work_part or accepted_title
    ...
    vals = {
        ...
        "legacy_counterparty_text": visible_labor_unit,
        ...
        "legacy_visible_counterparty": visible_labor_unit,
        "legacy_visible_project_name": visible_project_name,
        "legacy_visible_title": execution_title(visible_title) if visible_title else False,
        "legacy_visible_category": "劳务",
        ...
        "legacy_visible_created_time": created_time,
        ...
        "contract_payment_method_text": visible_payment_terms,
    ...
    if visible_push_project_name:
        vals["legacy_visible_contractor"] = visible_push_project_name
...
def main() -> None:
    ...
    if not category:
        raise RuntimeError({"missing_expense_contract_labor_category": True})
        ...
        detail.append(
            {
                ...
                "accepted_title": contract.legacy_visible_title,
    ...
    labor_count = Contract.search_count(projected_domain + [("expense_contract_category_id", "=", category.id)])
    ...
    result = {
        ...
        and labor_count == len(seen)
        and direct_count == len(seen)
        and wrapper_attachment_count >= source_attachment_count
        else "FAIL",
        "mode": "direct_acceptance_labor_contract_expense_execution_write",
        ...
        "labor_category_count": labor_count,
    ...
    print("DIRECT_ACCEPTANCE_LABOR_CONTRACT_EXPENSE_EXECUTION_WRITE=" + json.dumps(result, ensure_ascii=False, sort_keys=True))
