"""Authorized project v4 200-row create-only write for Odoo shell."""
...
INPUT_CSV = Path("/mnt/artifacts/migration/project_next_200_candidate_v4.csv")
PACKET_JSON = Path("/mnt/artifacts/migration/project_v4_200_write_authorization_packet.json")
PRE_WRITE_SNAPSHOT_CSV = Path("/mnt/artifacts/migration/project_v4_200_pre_write_snapshot.csv")
...
def main():
    ...
    payload = {
        ...
        "mode": "project_v4_200_create_only_write",
    ...
    print(
        "PROJECT_V4_200_WRITE_RESULT="
        + json.dumps(payload["summary"] | {"status": payload["status"]}, ensure_ascii=False, sort_keys=True)
    )
    if post_errors:
        raise RuntimeError({"project_v4_write_post_errors": post_errors})
