"""Create partner records for missing-tax single-source company rows."""
...
DESIGN_CSV = Path("/mnt/artifacts/migration/partner_l4_missing_tax_single_source_write_design_rows_v1.csv")
PRE_WRITE_SNAPSHOT_CSV = Path("/mnt/artifacts/migration/partner_l4_missing_tax_single_source_1554_pre_write_snapshot_v1.csv")
POST_WRITE_SNAPSHOT_CSV = Path("/mnt/artifacts/migration/partner_l4_missing_tax_single_source_1554_post_write_snapshot_v1.csv")
WRITE_RESULT_JSON = Path("/mnt/artifacts/migration/partner_l4_missing_tax_single_source_1554_write_result_v1.json")
ROLLBACK_TARGET_CSV = Path("/mnt/artifacts/migration/partner_l4_missing_tax_single_source_1554_rollback_targets_v1.csv")
EXPECTED_COUNT = 1554
RUN_ID = "ITER-2026-04-14-PARTNER-L4-MISSING-TAX-SINGLE-SOURCE-1554-WRITE"
...
def build_create_vals(row):
    ...
    return {
        ...
        "legacy_source_evidence": "partner_l4_missing_tax_single_source_write_design_v1",
    }
...
def main():
    ...
    for index, row in enumerate(rows, start=2):
        row_errors = []
        if clean(row.get("proposed_action")) != "create_company_partner_without_tax_number":
            row_errors.append("proposed_action_not_create_company_partner_without_tax_number")
        ...
        if clean(row.get("target_customer_rank")) != "1" or clean(row.get("target_supplier_rank")) != "0":
            row_errors.append("target_rank_not_customer_one_supplier_zero")
    ...
    payload = {
        ...
        "mode": "partner_missing_tax_single_source_create_write",
    ...
    print("PARTNER_MISSING_TAX_SINGLE_SOURCE_WRITE_RESULT=" + json.dumps({
    ...
    if post_errors:
        raise RuntimeError({"partner_missing_tax_single_source_write_failed": post_errors})
