    ...
    try:
        ...
        package_verify = run(
            [
                ...
                "--package-root",
                "artifacts/migration/partner_business_aligned_replay_package_v1",
            ]
        )
        ...
        expect_equal(errors, "business_aligned_payload_rows", business.get("business_aligned_payload_rows"), 8777)
        ...
        expect_equal(errors, "gate_write_candidate", gate_counts.get("write_candidate"), 1667)
        expect_equal(errors, "gate_update_only_candidate", gate_counts.get("update_only_candidate"), 5175)
        expect_equal(errors, "gate_blocked_review", gate_counts.get("blocked_review"), 1935)
        expect_equal(errors, "review_status", review.get("status"), "PASS")
        expect_equal(errors, "review_rows", review.get("review_rows"), 1935)
        expect_equal(errors, "package_status", package.get("status"), "PASS")
        if int(package.get("file_count") or 0) < 41:
            errors.append({"check": "package_file_count_min", "actual": package.get("file_count"), "expected_min": 41})
        if errors:
        ...
        result = {
            ...
            "package_verify": package_verify.strip(),
            "db_writes": 0,
        }
