"""Inlined writer for the next500h project_member neutral carrier."""
...
PRE_VISIBILITY_JSON = Path("/mnt/artifacts/migration/project_member_duplicate_relation_next500h_pre_visibility_v1.json")
POST_VISIBILITY_JSON = Path("/mnt/artifacts/migration/project_member_duplicate_relation_next500h_post_visibility_v1.json")
WRITE_RESULT_JSON = Path("/mnt/artifacts/migration/project_member_duplicate_relation_next500h_write_result_v1.json")
ROLLBACK_TARGET_CSV = Path("/mnt/artifacts/migration/project_member_duplicate_relation_next500h_rollback_targets_v1.csv")

RUN_ID = "ITER-2026-04-15-1990N"
...
def main():
    ...
    payload = {
        ...
        "mode": "project_member_duplicate_relation_neutral_next500h_write",
    ...
    print("PROJECT_MEMBER_DUPLICATE_RELATION_NEUTRAL_NEXT500H_WRITE=" + json.dumps({
    ...
    if post_errors:
        raise RuntimeError({"duplicate_relation_neutral_next500h_write_failed": post_errors})
