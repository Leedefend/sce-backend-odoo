"""Inlined writer for the final355 project_member neutral carrier."""
...
PRE_VISIBILITY_JSON = Path("/mnt/artifacts/migration/project_member_duplicate_relation_final355_pre_visibility_v1.json")
POST_VISIBILITY_JSON = Path("/mnt/artifacts/migration/project_member_duplicate_relation_final355_post_visibility_v1.json")
WRITE_RESULT_JSON = Path("/mnt/artifacts/migration/project_member_duplicate_relation_final355_write_result_v1.json")
ROLLBACK_TARGET_CSV = Path("/mnt/artifacts/migration/project_member_duplicate_relation_final355_rollback_targets_v1.csv")

RUN_ID = "ITER-2026-04-15-2002N"
...
def main():
    ...
    if len(rows) != EXPECTED_ROWS:
        errors.append({"error": "slice_row_count_not_355", "rows": len(rows)})
    ...
    if len(created_rows) != EXPECTED_ROWS:
        post_errors.append({"error": "created_count_not_355", "created": len(created_rows)})
    if len(rollback_rows) != EXPECTED_ROWS:
        post_errors.append({"error": "rollback_target_count_not_355", "rows": len(rollback_rows)})
    if visibility_changed:
        post_errors.append({"error": "project_visibility_changed"})

    payload = {
        "status": "PASS" if not post_errors else "FAIL",
        "mode": "project_member_duplicate_relation_neutral_final355_write",
    ...
    print("PROJECT_MEMBER_DUPLICATE_RELATION_NEUTRAL_FINAL355_WRITE=" + json.dumps({
    ...
    if post_errors:
        raise RuntimeError({"duplicate_relation_neutral_final355_write_failed": post_errors})
