...
def loan_vals(seq: int, spec: dict[str, Any], row: dict[str, Any], key: str) -> dict[str, Any]:
    ...
    return {
        ...
        "financing_loan_request_department": clean(row.get("SQBM")),
        "financing_loan_request_time": clean(row.get("SQSJ") or row.get("TXRQ") or row.get("LRSJ")),
        "financing_loan_budget_included": clean(row.get("SFYSN")),
        "financing_loan_actual_loan_amount": visible_number(row.get("JKJE") or row.get("DKJE")),
        "financing_loan_fund_usage_plan": clean(row.get("ZYZJSYAP") or row.get("YT") or row.get("BZ")),
        "financing_loan_bank_name": clean(row.get("KHYH") or row.get("DKYH")),
        "financing_loan_borrow_account": clean(row.get("ZKZH") or row.get("DKZH")),
        "financing_loan_approved_amount": visible_number(row.get("SJPFJE") or row.get("JKJE") or row.get("DKJE")),
        "financing_loan_request_amount": visible_number(row.get("SQJE") or row.get("JKJE") or row.get("DKJE")),
        "financing_loan_expected_return_time": clean(row.get("YJGHSJ") or row.get("YDQX") or row.get("HKRQ")),
        "financing_loan_loan_type_display": clean(row.get("FKFSMC") or row.get("JKLX") or row.get("DKLX")),
