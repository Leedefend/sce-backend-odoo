"""Authorized project v5 200-row create-only write for Odoo shell."""
...
INPUT_CSV = Path("/mnt/artifacts/migration/project_next_200_candidate_v5.csv")
PACKET_JSON = Path("/mnt/artifacts/migration/project_v5_200_write_authorization_packet.json")
PRE_WRITE_SNAPSHOT_CSV = Path("/mnt/artifacts/migration/project_v5_200_pre_write_snapshot.csv")
POST_WRITE_SNAPSHOT_CSV = Path("/mnt/artifacts/migration/project_v5_200_post_write_snapshot.csv")
WRITE_RESULT_JSON = Path("/mnt/artifacts/migration/project_v5_200_write_result.json")
...
def main():
    ...
    if packet.get("payload_rows") != EXPECTED_COUNT:
        raise RuntimeError({"authorization_payload_not_200": packet.get("payload_rows")})
    ...
    if len(post_records) != EXPECTED_COUNT:
        post_errors.append("post_write_identity_count_not_200")
    ...
    payload = {
        ...
        "mode": "project_v5_200_create_only_write",
    ...
    print("PROJECT_V5_200_WRITE_RESULT=" + json.dumps(payload["summary"] | {"status": payload["status"]}, ensure_ascii=False, sort_keys=True))
    if post_errors:
        raise RuntimeError({"project_v5_write_post_errors": post_errors})
