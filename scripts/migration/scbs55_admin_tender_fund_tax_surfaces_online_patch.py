...
def vals_for(seq: int, spec: dict[str, Any], row: dict[str, Any], key: str) -> dict[str, Any]:
    if seq in (5, 6):
        return {
            ...
            "seal_use_date": parse_date(row.get("YYSJ")),
            "seal_department_name": clean(row.get("YYBM")),
            "seal_applicant_name": clean(row.get("YYSQR")),
            "seal_department_manager_sign": clean(row.get("YYBMFZRQZ")),
            "seal_type_name": clean(row.get("YYZL")),
            "seal_text": clean(row.get("YYWBMCJWH") or row.get("GZWJNBGY")),
            "seal_handler_sign": clean(row.get("JBRQZ")),
            "seal_leader_sign": clean(row.get("LDQZ")),
            "seal_copy_count": clean(row.get("FS")),
            "seal_return_date": parse_date(row.get("GHSJ")),
            "seal_contract_amount": amount(row.get("HTJE")),
            "seal_contract_no": clean(row.get("HTBH")),
            "seal_company_name": clean(row.get("SSGS")),
            "seal_using_company_name": clean(row.get("D_JCLY_SYYZGS")),
            "seal_take_out_flag": clean(row.get("D_JCLY_SFWD")),
    ...
    if seq == 15:
        return {
            ...
            "application_date": parse_date(row.get("SQRQ")),
            "borrow_project_name": clean(row.get("JYXMMC")),
            "borrow_department_name": clean(row.get("JYBMMC")),
            "borrower_name": clean(row.get("JYR")),
            "borrower_contact": clean(row.get("LXFS")),
            "borrow_form": clean(row.get("JYXS")),
            "responsible_person": clean(row.get("FZRMC")),
            "return_request_date": parse_date(row.get("GHSQRQ")),
            "return_apply_time": parse_dt(row.get("SQGHSJ")),
            "returned_flag": clean(row.get("SFGH")),
            "return_confirm_time": parse_dt(row.get("QRGHSJ")),
            "expected_return_date": parse_date(row.get("GHSQRQ") or row.get("GHRQ") or row.get("SQRQ")),
            "actual_return_date": parse_date(row.get("GHRQ")),
            "modifier_name": clean(row.get("XGR")),
            "modified_at": parse_dt(row.get("XGRQ")),
            "modify_note": clean(row.get("XGBZ")),
            "reviewer_name": clean(row.get("SDR")),
            "review_time": parse_dt(row.get("SDSJ")),
            "review_opinion": clean(row.get("SDYJ")),
            "legacy_document_no": clean(row.get("DJBH")),
            "legacy_document_state": clean(row.get("DJZT")),
            "legacy_source_table": spec["surface"],
            "legacy_source_id": key,
    ...
    if seq == 16:
        return {
            ...
            "state": "submitted",
            "applicant_name": clean(row.get("f_LRR")),
            "apply_date": parse_date(row.get("f_BMSJ")),
            "note": clean(row.get("TSJG")),
            "created_time": parse_dt(row.get("f_LRSJ") or row.get("f_BMSJ")),
