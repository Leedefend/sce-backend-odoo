...
run_check "S77 signup throttle records exist" "s77_data_dictionary_surface" \
  "select case when (select count(*) from sc_signup_throttle where key like 's77:signup:%') = 2 then 'ok' else 'S77 signup throttle records missing' end;" \
  "select id, key, count, window_start, last_seen from sc_signup_throttle where key like 's77:signup:%' order by key;"
