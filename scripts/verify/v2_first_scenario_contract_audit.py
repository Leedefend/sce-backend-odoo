...
SNAPSHOT = ROOT / "artifacts" / "v2" / "first_scenario_contract_snapshot_v1.json"
...
def run_audit() -> Dict[str, object]:
    ...
    if snapshot.get("snapshot_id") != "first_scenario_contract_snapshot_v1":
    ...
    success = dispatch_intent(
        intent="app.init",
        payload={"app_key": "platform", "model": "res.partner", "view_type": "form"},
        context={"user_id": 17, "company_id": 5, "trace_id": "scenario-1633-pass"},
    )
    ...
    chain_status = data.get("chain_status") if isinstance(data.get("chain_status"), dict) else {}
    missing_chain = _missing_keys(chain_status, snapshot.get("required_chain_status_fields") or [])
    if missing_chain:
        errors.append(f"missing_chain_status_fields:{','.join(missing_chain)}")

    if success.get("ok") is not True:
        errors.append("success_ok_not_true")
    if str(meta.get("intent") or "") != "app.init":
        errors.append("meta_intent_mismatch")
    if bool(chain_status.get("overall_ok")) is not True:
        errors.append("overall_chain_not_ok")

    failure = dispatch_intent(
        intent="app.init",
        payload={"model": ""},
        context={"user_id": 17, "company_id": 5, "trace_id": "scenario-1633-fail"},
    )
