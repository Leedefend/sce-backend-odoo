...
def run_audit() -> Dict[str, object]:
    ...
    from v2.intents.schemas.api_data_create_schema import ApiDataCreateRequestSchemaV2  # type: ignore

    registry = build_default_registry()
    try:
        entry = registry.get("api.data.create")
    except KeyError:
        entry = None
        errors.append("api_data_create_not_registered")

    if entry is not None:
        if str(getattr(entry, "canonical_intent", "")) != "api.data.create":
        ...
        for expected_tag in ("api", "data_create", "create", "registry_closure"):
            if expected_tag not in tags:
                errors.append(f"missing_tag:{expected_tag}")

        if "ApiDataCreateRequestSchemaV2" not in str(getattr(entry, "request_schema", "")):
            errors.append("request_schema_path_mismatch")
        if entry.handler_factory is not ApiDataCreateHandlerV2:
            errors.append("handler_factory_mismatch")

    try:
        schema_result = ApiDataCreateRequestSchemaV2.validate(
            {"model": "res.partner", "values": {"name": "Demo"}},
            {"trace_id": "api-data-create-reg-1650"},
        )
    ...
    if "api.data.create" not in migrated:
        errors.append("comparison_missing_migrated_api_data_create")
