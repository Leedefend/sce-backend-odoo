...
ADAPTER_FILE = ROOT / "addons" / "smart_core" / "core" / "industry_orchestration_service_adapter.py"
PROVIDER_FILE = ROOT / "addons" / "smart_construction_core" / "core_extension.py"
ARTIFACT_JSON = ROOT / "artifacts" / "orchestration_adapter_protocol_hook_guard.json"
...
def main() -> int:
    ...
    if missing:
        print("[orchestration_adapter_protocol_hook_guard] FAIL")
        for hook in missing:
            print(f"missing hook: {hook}")
        return 1

    print("[orchestration_adapter_protocol_hook_guard] PASS")
    return 0
