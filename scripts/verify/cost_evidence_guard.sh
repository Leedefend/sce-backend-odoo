...
OUT_JSON = Path("/mnt/artifacts/backend/cost_evidence_guard.json")
OUT_MD = Path("/mnt/artifacts/backend/cost_evidence_guard.md")
...
for name in PROJECT_NAMES:
    ...
    if not project:
        ...
        continue
    ledgers = CostLedger.search([("project_id", "=", project.id)])
    ...
    missing = [ledger.id for ledger in ledgers if ("project.cost.ledger", ledger.id) not in evidence_sources]
    report["projects"][name] = {
        "project_id": int(project.id),
        "cost_ledger_count": len(ledgers),
        "cost_evidence_count": len(evidences),
        "missing_ledger_ids": missing,
    }
    if missing:
        report["errors"].append(f"{name}: missing cost evidence for ledger ids {missing}")
...
OUT_MD.write_text(
    ...
    + "\n".join(
        f"- {name}: ledgers={row['cost_ledger_count']} evidences={row['cost_evidence_count']}"
...
if report["errors"]:
    print("[cost_evidence_guard] FAIL")
    for line in report["errors"]:
        print(f" - {line}")
    raise SystemExit(1)

print("[cost_evidence_guard] PASS")
PY
