...
def run_audit() -> Dict[str, object]:
    ...
    result = dispatch_intent(
        intent="ui.contract",
        payload={"model": " res.partner ", "view_type": "Tree"},
        context={"user_id": 7, "company_id": 2, "trace_id": "ui-exec-1630"},
    )
    ...
    if str(data.get("intent") or "") != "ui.contract":
        errors.append("intent_mismatch")
    if str(data.get("model") or "") != "res.partner":
        errors.append("schema_normalization_not_applied")
    if str(data.get("view_type") or "") != "tree":
        errors.append("view_type_normalization_not_applied")
