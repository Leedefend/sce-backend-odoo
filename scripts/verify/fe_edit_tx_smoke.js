...
function log(msg) {
  console.log(`[fe_edit_tx_smoke] ${msg}`);
}
...
async function main() {
  ...
  const meta = writeResp.body.meta || {};
  summary.push(`record_id: ${targetId}`);
  summary.push(`trace_id: ${meta.trace_id || '-'}`);
  summary.push(`write_mode: ${meta.write_mode || '-'}`);
  ...
  log('PASS edit_tx dry_run');
  log(`artifacts: ${outDir}`);
}

main().catch((err) => {
  console.error(`[fe_edit_tx_smoke] FAIL: ${err.message}`);
  process.exit(1);
});
