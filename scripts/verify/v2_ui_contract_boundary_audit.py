...
def run_audit() -> Dict[str, object]:
    ...
    from v2.builders.ui_contract_response_builder import UIContractResponseBuilderV2  # type: ignore
    from v2.contracts.results import UIContractResultV2  # type: ignore
    ...
    from v2.services.ui_contract_service import UIContractServiceV2  # type: ignore

    handler = UIContractHandlerV2()
    if not isinstance(getattr(handler, "_service", None), UIContractServiceV2):
        errors.append("handler_missing_service_dependency")
    if not isinstance(getattr(handler, "_builder", None), UIContractResponseBuilderV2):
        errors.append("handler_missing_builder_dependency")

    service = UIContractServiceV2()
    result_object = service.build_contract_stub(
        payload={"model": "res.partner", "view_type": "form", "schema_validated": True},
        context={"trace_id": "ui-boundary-1631", "user_id": 9, "company_id": 3},
    )
    if not isinstance(result_object, UIContractResultV2):
        errors.append("service_not_return_result_object")

    builder = UIContractResponseBuilderV2()
    ...
    dispatch_result = dispatch_intent(
        ...
        context={"user_id": 11, "company_id": 4, "trace_id": "ui-boundary-dispatch-1631"},
    )
