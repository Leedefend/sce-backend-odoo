...
def run_audit() -> Dict[str, object]:
    ...
    from v2.intents.schemas.api_data_unlink_schema import ApiDataUnlinkRequestSchemaV2  # type: ignore

    registry = build_default_registry()
    try:
        entry = registry.get("api.data.unlink")
    except KeyError:
        entry = None
        errors.append("api_data_unlink_not_registered")

    if entry is not None:
        if str(getattr(entry, "canonical_intent", "")) != "api.data.unlink":
        ...
        for expected_tag in ("api", "data_unlink", "unlink", "registry_closure"):
            if expected_tag not in tags:
                errors.append(f"missing_tag:{expected_tag}")

        if "ApiDataUnlinkRequestSchemaV2" not in str(getattr(entry, "request_schema", "")):
            errors.append("request_schema_path_mismatch")
        if entry.handler_factory is not ApiDataUnlinkHandlerV2:
            errors.append("handler_factory_mismatch")

    try:
        schema_result = ApiDataUnlinkRequestSchemaV2.validate(
            {"model": "res.partner", "ids": [1, 2]},
            {"trace_id": "api-data-unlink-reg-1654"},
        )
    ...
    if "api.data.unlink" not in migrated:
        errors.append("comparison_missing_migrated_api_data_unlink")
