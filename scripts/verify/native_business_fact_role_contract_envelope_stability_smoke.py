...
def _envelope_signature(payload: dict) -> tuple[tuple[str, ...], tuple[str, ...], tuple[str, ...], tuple[str, ...]]:
    root_keys = tuple(sorted(str(key) for key in payload.keys()))
    data = payload.get("data") if isinstance(payload.get("data"), dict) else {}
    data_keys = tuple(sorted(str(key) for key in data.keys()))
    ...
    policy_keys = tuple(sorted(str(key) for key in policy.keys()))
    return root_keys, data_keys, meta_keys, policy_keys
...
def main() -> None:
    ...
    for role_key, login, password in roles:
        ...
        for model in models:
            ...
            sig1 = _envelope_signature(p1)
            sig2 = _envelope_signature(p2)
            if sig1 != sig2:
                raise RuntimeError(f"{role_key} {model} envelope drift")

        details.append(f"{role_key}:{login}")

    print(
        "[native_business_fact_role_contract_envelope_stability_smoke] "
