...
def run_audit() -> Dict[str, object]:
    ...
    from v2.intents.schemas.execute_button_schema import ExecuteButtonRequestSchemaV2  # type: ignore

    registry = build_default_registry()
    try:
        entry = registry.get("execute_button")
    except KeyError:
        entry = None
        errors.append("execute_button_not_registered")

    if entry is not None:
        if str(getattr(entry, "canonical_intent", "")) != "execute_button":
            errors.append("canonical_intent_mismatch")
        if str(getattr(entry, "intent_class", "")) != "domain":
            errors.append("intent_class_mismatch")

        tags = tuple(getattr(entry, "tags", ()) or ())
        for expected_tag in ("domain", "action", "button", "write_path"):
            if expected_tag not in tags:
                errors.append(f"missing_tag:{expected_tag}")

        if "ExecuteButtonRequestSchemaV2" not in str(getattr(entry, "request_schema", "")):
            errors.append("request_schema_path_mismatch")
        if entry.handler_factory is not ExecuteButtonHandlerV2:
            errors.append("handler_factory_mismatch")

    try:
        schema_result = ExecuteButtonRequestSchemaV2.validate(
            {"model": "res.partner", "record_id": 1, "button_name": "action_confirm"},
            {"trace_id": "execute-reg-1634"},
        )
    ...
    if "execute_button" not in migrated:
        errors.append("comparison_missing_migrated_execute_button")
