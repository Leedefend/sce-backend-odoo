...
def _login_token_with_fallback(intent_url: str, db_name: str) -> tuple[str, str]:
    ...
    raw_candidates = str(
        os.getenv("E2E_ROLE_MATRIX_LOGINS") or "demo_pm,demo_finance,demo_role_executive,admin"
    )
    candidates = [item.strip() for item in raw_candidates.split(",") if item.strip()]
    if explicit_login and explicit_login not in candidates:
        candidates.insert(0, explicit_login)
    elif explicit_login:
...
def main() -> None:
    ...
    probe_login, token = _login_token_with_fallback(intent_url, db_name)
