...
def run_audit() -> Dict[str, object]:
    ...
    from v2.intents.schemas.api_onchange_schema import ApiOnchangeRequestSchemaV2  # type: ignore

    registry = build_default_registry()
    try:
        entry = registry.get("api.onchange")
    except KeyError:
        entry = None
        errors.append("api_onchange_not_registered")

    if entry is not None:
        if str(getattr(entry, "canonical_intent", "")) != "api.onchange":
        ...
        for expected_tag in ("api", "onchange", "field_linkage", "registry_closure"):
            if expected_tag not in tags:
                errors.append(f"missing_tag:{expected_tag}")

        if "ApiOnchangeRequestSchemaV2" not in str(getattr(entry, "request_schema", "")):
            errors.append("request_schema_path_mismatch")
        if entry.handler_factory is not ApiOnchangeHandlerV2:
            errors.append("handler_factory_mismatch")

    try:
        schema_result = ApiOnchangeRequestSchemaV2.validate(
            {"model": "res.partner", "field_name": "name"},
            {"trace_id": "api-onchange-reg-1642"},
        )
    ...
    if "api.onchange" not in migrated:
        errors.append("comparison_missing_migrated_api_onchange")
