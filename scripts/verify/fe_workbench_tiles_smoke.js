...
function log(msg) {
  console.log(`[fe_workbench_tiles_smoke] ${msg}`);
}
...
async function main() {
  ...
  if (!tileScenes.length) {
    throw new Error('no scenes with tiles');
  }

  log('PASS workbench tiles');
  log(`artifacts: ${outDir}`);
}

main().catch((err) => {
  console.error(`[fe_workbench_tiles_smoke] FAIL: ${err.message}`);
  process.exit(1);
});
