...
def run_audit() -> Dict[str, object]:
    ...
    from v2.intents.schemas.load_contract_schema import LoadContractRequestSchemaV2  # type: ignore

    registry = build_default_registry()
    try:
        entry = registry.get("load_contract")
    except KeyError:
        entry = None
        errors.append("load_contract_not_registered")

    if entry is not None:
        if str(getattr(entry, "canonical_intent", "")) != "load_contract":
        ...
        for expected_tag in ("ui", "load_contract", "contract", "registry_closure"):
            if expected_tag not in tags:
                errors.append(f"missing_tag:{expected_tag}")

        if "LoadContractRequestSchemaV2" not in str(getattr(entry, "request_schema", "")):
            errors.append("request_schema_path_mismatch")
        if entry.handler_factory is not LoadContractHandlerV2:
            errors.append("handler_factory_mismatch")

    try:
        schema_result = LoadContractRequestSchemaV2.validate(
            {"model": "res.partner", "view_type": "form"},
            {"trace_id": "load-contract-reg-1666"},
        )
    ...
    if "load_contract" not in migrated:
        errors.append("comparison_missing_migrated_load_contract")
