...
function log(msg) {
  console.log(`[fe_scene_targets_resolve_smoke] ${msg}`);
}
...
async function main() {
  ...
  targets.forEach((key) => {
    ...
    const menuId = target.menu_id;
    if (!actionId || !menuId) {
      errors.push(`${key}: target action_id/menu_id missing`);
    }
  });
...
main().catch((err) => {
  console.error(`[fe_scene_targets_resolve_smoke] FAIL: ${err.message}`);
  process.exit(1);
});
