...
OUT_JSON = ROOT / "artifacts" / "backend" / "product_payment_entry_contract_guard.json"
...
def main() -> int:
    ...
    try:
        ...
        status, create_resp = _post(
            ...
            {
                "name": f"FR4-PAYMENT-ENTRY-{uuid4().hex[:8]}",
                "description": "fr4 payment entry contract guard",
        ...
        status, payment_resp = _post(
            ...
            "payment.record.create",
            {
                ...
                "date": "2026-03-23",
                "amount": "4321.00",
                "description": "FR-4 prepared payment entry",
            },
            db_name=db_name,
        )
        _assert_ok(status, payment_resp, "payment.record.create")
        data = payment_resp.get("data") if isinstance(payment_resp.get("data"), dict) else {}
        ...
        if int(data.get("payment_request_id") or 0) <= 0:
            raise RuntimeError("payment_request_id missing")
        if str(data.get("state") or "").strip() != "draft":
        ...
        print("[product_payment_entry_contract_guard] FAIL")
        print(f" - {exc}")
        return 1

    _write_json(OUT_JSON, report)
    print("[product_payment_entry_contract_guard] PASS")
    return 0
