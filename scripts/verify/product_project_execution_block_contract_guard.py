...
def main() -> int:
    ...
    try:
        ...
        status, create_resp = _post(intent_url, token, "project.initiation.enter", {
            "name": f"P13B-EXEC-BLOCK-{uuid4().hex[:8]}",
            "description": "execution block contract guard",
        ...
        for block_key, expected_type in SUPPORTED.items():
            status, block_resp = _post(intent_url, token, "project.execution.block.fetch", {"project_id": project_id, "block_key": block_key}, db_name=db_name)
            _assert_ok(status, block_resp, f"project.execution.block.fetch({block_key})")
        ...
        print("[product_project_execution_block_contract_guard] FAIL")
        print(f" - {exc}")
        return 1

    _write_json(OUT_JSON, report)
    print("[product_project_execution_block_contract_guard] PASS")
    return 0
