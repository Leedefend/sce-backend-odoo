...
def run_audit() -> Dict[str, object]:
    ...
    from v2.intents.schemas.describe_model_schema import MetaDescribeModelRequestSchemaV2  # type: ignore

    registry = build_default_registry()
    try:
        entry = registry.get("meta.describe_model")
    except KeyError:
        entry = None
        errors.append("meta_describe_model_not_registered")

    if entry is not None:
        if str(getattr(entry, "canonical_intent", "")) != "meta.describe_model":
        ...
        for expected_tag in ("meta", "model", "schema", "describe"):
            if expected_tag not in tags:
                errors.append(f"missing_tag:{expected_tag}")

        if "MetaDescribeModelRequestSchemaV2" not in str(getattr(entry, "request_schema", "")):
            errors.append("request_schema_path_mismatch")

        if entry.handler_factory is not MetaDescribeModelHandlerV2:
            errors.append("handler_factory_mismatch")

    try:
        schema_result = MetaDescribeModelRequestSchemaV2.validate({"model": "res.partner"}, {"trace_id": "meta-reg-1625"})
    ...
    if "meta.describe_model" not in migrated:
        errors.append("comparison_missing_migrated_meta_describe_model")
