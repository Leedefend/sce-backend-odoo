...
OUT_JSON = ROOT / "artifacts" / "backend" / "product_project_dashboard_entry_contract_guard.json"

ENTRY_KEYS = {"project_id", "title", "summary", "blocks", "suggested_action", "runtime_fetch_hints"}
SUMMARY_KEYS = {"project_code", "manager_name", "partner_name", "stage_name", "health_state"}
BLOCK_KEYS = {"progress", "risks", "next_actions"}
...
def main() -> int:
    ...
    try:
        ...
        status, create_resp = _post(intent_url, token, "project.initiation.enter", {
            "name": f"P12E-E5-ENTRY-{uuid4().hex[:8]}",
            "description": "dashboard entry contract guard",
        ...
        status, entry_resp = _post(intent_url, token, "project.dashboard.enter", {"project_id": project_id}, db_name=db_name)
        _assert_ok(status, entry_resp, "project.dashboard.enter")
        ...
        print("[product_project_dashboard_entry_contract_guard] FAIL")
        print(f" - {exc}")
        return 1

    _write_json(OUT_JSON, report)
    print("[product_project_dashboard_entry_contract_guard] PASS")
    return 0
