...
def _extract_home_blocks(payload: dict) -> list[dict]:
...
def main() -> None:
    ...
    if uid <= 0:
        raise SystemExit("[native_business_admin_config_home_block_intent_parity_verify] FAIL admin auth failed")

    suffix = str(int(time.time()))
    create_vals = [
        {
            "type": "home_block",
            "code": f"project.dashboard.{suffix}",
            "name": "项目首页区块",
            "scope_type": "role",
            "scope_ref": "project_manager",
            "value_json": {"block_key": f"project.dashboard.{suffix}", "is_enabled": True},
        },
        {
            "type": "home_block",
            ...
            "scope_type": "role",
            "scope_ref": "finance",
            "value_json": {"block_key": f"payment.request.{suffix}", "is_enabled": True},
        },
    ]
    ...
    try:
        ...
        for vals in create_vals:
            ...
            if rec_id <= 0:
                raise SystemExit(
                    "[native_business_admin_config_home_block_intent_parity_verify] FAIL create home_block failed"
                )
        ...
        if status != 200 or not isinstance(body, dict) or body.get("ok") is not True:
            raise SystemExit(
                "[native_business_admin_config_home_block_intent_parity_verify] FAIL "
                f"system.init not ok status={status}"
            )

        home_blocks = _extract_home_blocks(body)
        if not home_blocks:
            raise SystemExit(
                "[native_business_admin_config_home_block_intent_parity_verify] FAIL home_blocks missing in system.init"
            )
        ...
        print(
            "[native_business_admin_config_home_block_intent_parity_verify] PASS "
            f"uid={uid} expected_pairs={len(expected)} role_groups={len(home_blocks)}"
        )
    finally:
        session.call_kw(
            "ir.config_parameter",
