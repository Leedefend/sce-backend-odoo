...
ensure_module_installed() {
  ...
  echo "[capability_lint] DB ${DB_NAME} missing module ${module} (state=${state})"
  if [ "${E2E_AUTO_INSTALL:-}" = "1" ]; then
    echo "[capability_lint] auto-install enabled, installing ${module}"
    CODEX_NEED_UPGRADE=1 MODULE="${module}" ODOO_ARGS="-i ${module}" make mod.upgrade
    make restart
    return 0
  fi

  echo "[capability_lint] run: CODEX_NEED_UPGRADE=1 MODULE=${module} ODOO_ARGS=\"-i ${module}\" make mod.upgrade"
  return 2
}
