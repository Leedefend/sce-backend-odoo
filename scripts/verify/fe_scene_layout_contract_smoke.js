...
function log(msg) {
  console.log(`[fe_scene_layout_contract_smoke] ${msg}`);
}
...
function validateLayout(layout, issues) {
  ...
  if (!['list', 'record', 'workspace', 'ledger'].includes(kind)) {
    issues.push(`layout.kind invalid: ${kind}`);
  }
  if (!['fixed', 'scroll'].includes(sidebar)) {
    issues.push(`layout.sidebar invalid: ${sidebar}`);
  }
  if (!['compact', 'full'].includes(header)) {
    issues.push(`layout.header invalid: ${header}`);
  }
}
...
async function main() {
  ...
  scenes.forEach((scene) => {
    const code = scene.code || scene.key || 'unknown';
    const errs = [];
    validateLayout(scene.layout, errs);
    if (errs.length) {
      issues.push(`${code}: ${errs.join(', ')}`);
    }
  });

  if (issues.length) {
    writeSummary(summary.concat(['layout_issues:', ...issues.map((line) => `- ${line}`)]));
    throw new Error(`scene layout issues: ${issues.length}`);
  }

  log('PASS scene layout contract');
  log(`artifacts: ${outDir}`);
}

main().catch((err) => {
  console.error(`[fe_scene_layout_contract_smoke] FAIL: ${err.message}`);
  process.exit(1);
});
