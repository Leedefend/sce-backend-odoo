...
function log(msg) {
  console.log(`[fe_scene_default_sort_smoke] ${msg}`);
}
...
async function main() {
  ...
  if (!scenes.length && nav.length > 0) {
    ...
    log('SKIP default_sort smoke (scenes missing, nav fallback present)');
...
main().catch((err) => {
  console.error(`[fe_scene_default_sort_smoke] FAIL: ${err.message}`);
  process.exit(1);
});
