...
SNAPSHOT = ROOT / "artifacts" / "v2" / "load_contract_contract_snapshot_v1.json"
...
def run_audit() -> Dict[str, object]:
    ...
    if snapshot.get("snapshot_id") != "load_contract_contract_snapshot_v1":
    ...
    success = dispatch_intent(
        ...
        context={"user_id": 17, "company_id": 5, "trace_id": "load-contract-contract-1669-pass"},
    )
    ...
    if str(meta.get("intent") or "") != "load_contract":
    ...
    failure = dispatch_intent(
        intent="load_contract",
        payload={"model": "", "view_type": "form"},
        context={"user_id": 17, "company_id": 5, "trace_id": "load-contract-contract-1669-fail"},
    )
