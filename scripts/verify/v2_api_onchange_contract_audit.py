...
SNAPSHOT = ROOT / "artifacts" / "v2" / "api_onchange_contract_snapshot_v1.json"
...
def run_audit() -> Dict[str, object]:
    ...
    if snapshot.get("snapshot_id") != "api_onchange_contract_snapshot_v1":
    ...
    success = dispatch_intent(
        ...
        context={"user_id": 17, "company_id": 5, "trace_id": "api-onchange-contract-1645-pass"},
    )
    ...
    if str(meta.get("intent") or "") != "api.onchange":
    ...
    failure = dispatch_intent(
        intent="api.onchange",
        payload={"model": "", "field_name": "name"},
        context={"user_id": 17, "company_id": 5, "trace_id": "api-onchange-contract-1645-fail"},
    )
