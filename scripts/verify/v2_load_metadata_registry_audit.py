...
def run_audit() -> Dict[str, object]:
    ...
    from v2.handlers.meta.load_metadata import LoadMetadataHandlerV2  # type: ignore
    from v2.intents.registry import build_default_registry  # type: ignore
    from v2.intents.schemas.load_metadata_schema import LoadMetadataRequestSchemaV2  # type: ignore

    registry = build_default_registry()
    try:
        entry = registry.get("load_metadata")
    except KeyError:
        entry = None
        errors.append("load_metadata_not_registered")

    if entry is not None:
        if str(getattr(entry, "canonical_intent", "")) != "load_metadata":
            errors.append("canonical_intent_mismatch")
        if str(getattr(entry, "intent_class", "")) != "meta":
            errors.append("intent_class_mismatch")

        tags = tuple(getattr(entry, "tags", ()) or ())
        for expected_tag in ("meta", "load_metadata", "schema", "registry_closure"):
            if expected_tag not in tags:
                errors.append(f"missing_tag:{expected_tag}")

        if "LoadMetadataRequestSchemaV2" not in str(getattr(entry, "request_schema", "")):
            errors.append("request_schema_path_mismatch")
        if entry.handler_factory is not LoadMetadataHandlerV2:
            errors.append("handler_factory_mismatch")

    try:
        schema_result = LoadMetadataRequestSchemaV2.validate(
            {"model": "res.partner"},
            {"trace_id": "load-metadata-reg-1670"},
        )
    ...
    if "load_metadata" not in migrated:
        errors.append("comparison_missing_migrated_load_metadata")
