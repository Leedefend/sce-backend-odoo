...
def run_audit() -> Dict[str, object]:
    ...
    from v2.intents.schemas.file_download_schema import FileDownloadRequestSchemaV2  # type: ignore

    registry = build_default_registry()
    try:
        entry = registry.get("file.download")
    except KeyError:
        entry = None
        errors.append("file_download_not_registered")

    if entry is not None:
        if str(getattr(entry, "canonical_intent", "")) != "file.download":
        ...
        for expected_tag in ("api", "file_download", "download", "registry_closure"):
            if expected_tag not in tags:
                errors.append(f"missing_tag:{expected_tag}")

        if "FileDownloadRequestSchemaV2" not in str(getattr(entry, "request_schema", "")):
            errors.append("request_schema_path_mismatch")
        if entry.handler_factory is not FileDownloadHandlerV2:
            errors.append("handler_factory_mismatch")

    try:
        schema_result = FileDownloadRequestSchemaV2.validate(
            {
                "model": "res.partner",
                "res_id": 1,
                "name": "demo.txt"
            },
            {"trace_id": "file-download-reg-1662"},
        )
    ...
    if "file.download" not in migrated:
        errors.append("comparison_missing_migrated_file_download")
