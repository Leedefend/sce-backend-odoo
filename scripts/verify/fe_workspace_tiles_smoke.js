...
function log(msg) {
  console.log(`[fe_workspace_tiles_smoke] ${msg}`);
}
...
async function main() {
  ...
  if (!defaultScene) {
    throw new Error('default scene missing');
  }
  if (!tiles.length) {
    throw new Error('default scene tiles missing');
  }

  log('PASS workspace tiles');
  log(`artifacts: ${outDir}`);
}

main().catch((err) => {
  console.error(`[fe_workspace_tiles_smoke] FAIL: ${err.message}`);
  process.exit(1);
});
