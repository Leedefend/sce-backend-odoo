...
FORBIDDEN_PHRASES = {
    ...
    "表单配置审计失败": "正式用户面必须使用配置检查口径",
    "原生表单": "正式用户面必须使用标准表单口径",
    "治理表单": "正式用户面必须使用受控表单口径",
    "动作页面": "正式用户面必须使用操作页面口径",
    "原生界面": "正式用户面必须使用标准界面口径",
    "治理界面": "正式用户面必须使用受控界面口径",
    "视图编排": "正式用户面必须使用页面编排口径",
    "沉淀到用户模块": "菜单配置说明必须使用正式菜单基线口径",
    "物理删除": "菜单删除说明不能暴露物理删除口径",
    "按动作": "正式用户面必须使用业务操作口径",
    "整改动作": "正式用户面必须使用整改项口径",
    "当前模型": "正式用户面必须使用当前业务对象口径",
    "覆盖扫描": "正式用户面必须使用覆盖检查口径",
    "全部模型": "正式用户面必须使用全部业务对象口径",
    "暂不支持此操作": "正式用户面必须使用当前操作暂不可用口径",
    "网络异常": "正式用户面必须使用网络连接问题口径",
    "系统异常": "正式用户面必须使用系统处理问题口径",
    "系统处理异常": "正式用户面必须使用系统处理问题口径",
    "功能依赖异常": "正式用户面必须使用功能依赖问题口径",
    "入口配置异常": "正式用户面必须使用入口配置需要完善口径",
    "页面结构暂不支持": "正式用户面必须使用页面结构待完善口径",
    "请联系管理员": "正式用户面必须使用系统管理员口径",
    "请检查浏览器剪贴板权限": "正式用户面必须使用允许剪贴板访问口径",
    "联系管理员": "正式用户面必须使用系统管理员口径",
    "系统服务处理异常": "正式用户面必须使用系统服务处理问题口径",
    "系统服务异常": "正式用户面必须使用系统服务处理问题口径",
    "页面动作缺少 action_id": "正式用户面不能暴露 action_id 口径",
    "缺少 action_id": "正式用户面不能暴露 action_id 口径",
    "页面动作缺少 model": "正式用户面不能暴露 model 口径",
    "缺少 model": "正式用户面不能暴露 model 口径",
    "当前动作需要记录上下文": "正式用户面必须使用先选择记录口径",
    "当前操作暂不支持": "正式用户面必须使用当前操作暂不可用口径",
    "缺少能力权限": "正式用户面必须使用权限未开通口径",
    "场景配置异常": "正式用户面必须使用场景配置问题口径",
    "场景注册校验失败": "正式用户面必须使用场景配置校验口径",
    "导航异常": "正式用户面必须使用导航待恢复口径",
    "原生明细": "正式用户面必须使用页面明细口径",
    "治理信息": "正式用户面必须使用配置信息口径",
    "治理操作": "正式用户面必须使用配置操作口径",
    "治理通道": "正式用户面必须使用发布通道口径",
    "治理门禁": "正式用户面必须使用发布门禁口径",
    "治理原因": "正式用户面必须使用配置原因口径",
    "治理质量": "正式用户面必须使用配置质量口径",
    "Action has no model": "正式用户面不能暴露英文业务对象缺失错误",
    "页面动作": "正式用户面必须使用页面操作口径",
    "负责人候选加载受限（": "正式用户面不能暴露业务对象/操作编码",
    "重复请求被忽略": "正式用户面必须使用重复提交已忽略口径",
    "已幂等处理": "正式用户面必须使用已处理口径",
    "本次请求": "正式用户面必须使用本次操作口径",
    "请求的资源": "正式用户面必须使用要访问的内容口径",
    "请求处理失败": "正式用户面必须使用处理失败口径",
