...
const outDir = path.join(ARTIFACTS_DIR, 'codex', 'payment-slice-browser-smoke', ts);
...
function log(message) {
  console.log(`[payment_slice_browser_smoke] ${message}`);
}
...
try {
  ...
  const projectName = `FR4-PAYMENT-${Date.now()}`;
  ...
  writeJson('execution_snapshot.json', await snapshot(page));
  ...
  writeJson('cost_snapshot.json', await snapshot(page));
  ...
  await costForm.locator('input[type="text"]').fill('FR-4 browser smoke cost');
  ...
  writeJson('payment_snapshot.json', await snapshot(page));
  ...
  await paymentForm.locator('input[type="text"]').fill('FR-4 browser smoke payment');
  await paymentForm.getByRole('button', { name: '记录付款' }).click();
  const paymentFeedback = await waitForFeedback(page, '付款记录已创建');
  assert(paymentFeedback.includes('已创建') || paymentFeedback.includes('draft payment.request'), `unexpected payment feedback: ${paymentFeedback}`);

  await refreshBlockCard(page, '付款记录');
  await refreshBlockCard(page, '付款汇总');
  await page.waitForTimeout(1500);
  const bodyText = await page.locator('body').innerText();
  assert(bodyText.includes('FR-4 browser smoke payment'), 'payment list missing created record');
  assert(bodyText.includes('项目付款合计'), 'payment summary missing total label');
  assert(bodyText.includes('345.67') || bodyText.includes('345.7'), 'payment summary missing amount');

  const finalSnapshot = await snapshot(page);
  writeJson('final_snapshot.json', finalSnapshot);

  summary.cases.push({
    case_id: 'fr4_payment_slice_prepared_browser_smoke',
    status: 'PASS',
