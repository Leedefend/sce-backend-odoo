...
ENV=prod ENV_FILE=.env.prod expect_fail "seed.run without explicit db" PROFILE=base make seed.run DB_NAME=sc_demo
...
ENV=prod ENV_FILE=.env.prod SEED_DB_NAME_EXPLICIT=1 SEED_ALLOW_USERS_BOOTSTRAP=1 SC_BOOTSTRAP_USERS=1 PROFILE=base expect_no_guard "seed.run users_bootstrap with allow" make seed.run DB_NAME=sc_demo
