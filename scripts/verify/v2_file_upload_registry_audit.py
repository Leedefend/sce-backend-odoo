...
def run_audit() -> Dict[str, object]:
    ...
    from v2.intents.schemas.file_upload_schema import FileUploadRequestSchemaV2  # type: ignore

    registry = build_default_registry()
    try:
        entry = registry.get("file.upload")
    except KeyError:
        entry = None
        errors.append("file_upload_not_registered")

    if entry is not None:
        if str(getattr(entry, "canonical_intent", "")) != "file.upload":
        ...
        for expected_tag in ("api", "file_upload", "upload", "registry_closure"):
            if expected_tag not in tags:
                errors.append(f"missing_tag:{expected_tag}")

        if "FileUploadRequestSchemaV2" not in str(getattr(entry, "request_schema", "")):
            errors.append("request_schema_path_mismatch")
        if entry.handler_factory is not FileUploadHandlerV2:
            errors.append("handler_factory_mismatch")

    try:
        schema_result = FileUploadRequestSchemaV2.validate(
            {
                "model": "res.partner",
                "res_id": 1,
                "name": "demo.txt",
    ...
    if "file.upload" not in migrated:
        errors.append("comparison_missing_migrated_file_upload")
