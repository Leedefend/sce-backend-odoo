...
SCENE_PACKAGES_INSTALLED = ROOT / "addons/smart_core/handlers/scene_packages_installed.py"
...
def main() -> int:
    ...
    installed_text = (
        SCENE_PACKAGES_INSTALLED.read_text(encoding="utf-8", errors="ignore")
        if SCENE_PACKAGES_INSTALLED.is_file()
        else ""
    )
    ...
    if "call_extension_hook_first" in package_text:
        violations.append("scene_package handler still contains extension fallback hook")
    ...
    if "call_extension_hook_first" in gov_text:
        violations.append("scene_governance handler still contains extension fallback hook")
    if "smart_construction_scene.services.scene_package_service" not in installed_text:
        violations.append("scene_packages_installed handler missing direct scene package service path")
    if "call_extension_hook_first" in installed_text:
        violations.append("scene_packages_installed handler still contains extension fallback hook")
