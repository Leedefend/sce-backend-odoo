...
def _policy_signature(data: dict) -> tuple[str, str, tuple[str, ...], tuple[str, ...]]:
    policy = data.get("access_policy") if isinstance(data.get("access_policy"), dict) else {}
    mode = str(policy.get("mode") or "")
    reason = str(policy.get("reason_code") or "")
    blocked = tuple(sorted((policy.get("blocked_fields") or [])))
    degraded = tuple(sorted((policy.get("degraded_fields") or [])))
    return mode, reason, blocked, degraded
...
def main() -> None:
    ...
    for role_key, login, password in roles:
        ...
        for model in models:
            ...
            s1 = _policy_signature(d1)
            s2 = _policy_signature(d2)
            if s1 != s2:
                raise RuntimeError(f"{role_key} {model} policy surface drift: {s1} vs {s2}")
            if s1[0] != "allow":
                raise RuntimeError(f"{role_key} {model} policy mode unexpected: {s1[0]}")

        details.append(f"{role_key}:{login}")

    print(
        "[native_business_fact_role_policy_surface_consistency_smoke] "
