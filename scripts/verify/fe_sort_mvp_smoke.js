...
function log(msg) {
  console.log(`[fe_sort_mvp_smoke] ${msg}`);
}
...
async function main() {
  ...
  log('api.data.list (order)');
  const listPayload = {
    intent: 'api.data',
    params: { op: 'list', model: MODEL, fields: ['id', 'name', 'write_date'], order: 'name desc', limit: 5 },
  };
  ...
  summary.push(`order: name desc`);
  summary.push(`records: ${records.length}`);
...
main().catch((err) => {
  console.error(`[fe_sort_mvp_smoke] FAIL: ${err.message}`);
  process.exit(1);
});
