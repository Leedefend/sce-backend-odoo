...
function log(msg) {
  console.log(`[fe_scene_semantic_smoke] ${msg}`);
}
...
function validateSort(expr) {
  if (!expr || typeof expr !== 'string') return false;
  const parts = expr.trim().split(/\s+/);
  if (parts.length < 2) return false;
  const direction = parts[parts.length - 1].toLowerCase();
  if (!['asc', 'desc'].includes(direction)) return false;
  return parts.slice(0, -1).join(' ').length > 0;
}

function uniqueList(items) {
  return Array.isArray(items) ? new Set(items).size === items.length : true;
}
...
async function main() {
  ...
  scenes.forEach((scene) => {
    ...
    if (scene.default_sort && !validateSort(scene.default_sort)) {
      errors.push(`${scene.code || scene.key}: invalid default_sort=${scene.default_sort}`);
    }

    const lp = scene.list_profile || {};
    if (lp.columns) {
      if (!uniqueList(lp.columns)) {
        errors.push(`${scene.code || scene.key}: list_profile.columns has duplicates`);
      }
    }
    if (lp.hidden_columns) {
      if (!uniqueList(lp.hidden_columns)) {
        errors.push(`${scene.code || scene.key}: list_profile.hidden_columns has duplicates`);
      }
    }
    ...
    tiles.forEach((tile) => {
      ...
      const hasActionField = Object.prototype.hasOwnProperty.call(payload, 'action_xmlid') || Object.prototype.hasOwnProperty.call(payload, 'actionXmlid');
      const hasMenuField = Object.prototype.hasOwnProperty.call(payload, 'menu_xmlid') || Object.prototype.hasOwnProperty.call(payload, 'menuXmlid');
      if (hasActionField || hasMenuField) {
        const hasAction = !!payload.action_xmlid || !!payload.actionXmlid;
        const hasMenu = !!payload.menu_xmlid || !!payload.menuXmlid;
        if (!hasAction && !hasMenu) {
          errors.push(`${scene.code || scene.key}: tile ${tile.key} missing action/menu xmlid`);
        }
      }
    });
  });
...
main().catch((err) => {
  console.error(`[fe_scene_semantic_smoke] FAIL: ${err.message}`);
  process.exit(1);
});
