...
def run_audit() -> Dict[str, object]:
    ...
    from v2.intents.schemas.api_data_batch_schema import ApiDataBatchRequestSchemaV2  # type: ignore

    registry = build_default_registry()
    try:
        entry = registry.get("api.data.batch")
    except KeyError:
        entry = None
        errors.append("api_data_batch_not_registered")

    if entry is not None:
        if str(getattr(entry, "canonical_intent", "")) != "api.data.batch":
            errors.append("canonical_intent_mismatch")
        if str(getattr(entry, "intent_class", "")) != "api":
            errors.append("intent_class_mismatch")

        tags = tuple(getattr(entry, "tags", ()) or ())
        for expected_tag in ("api", "data_batch", "batch", "registry_closure"):
            if expected_tag not in tags:
                errors.append(f"missing_tag:{expected_tag}")

        if "ApiDataBatchRequestSchemaV2" not in str(getattr(entry, "request_schema", "")):
            errors.append("request_schema_path_mismatch")
        if entry.handler_factory is not ApiDataBatchHandlerV2:
            errors.append("handler_factory_mismatch")

    try:
        schema_result = ApiDataBatchRequestSchemaV2.validate(
            {"model": "res.partner", "operations": [{"op": "read"}]},
            {"trace_id": "api-data-batch-reg-1646"},
        )
    ...
    if "api.data.batch" not in migrated:
        errors.append("comparison_missing_migrated_api_data_batch")
