...
def main() -> int:
    ...
    try:
        ...
        policy = contract.get("product_policy") if isinstance(contract.get("product_policy"), dict) else {}
        ...
        _write(OUT_MD, f"# Delivery Policy Guard\n\n- status: `FAIL`\n- error: `{exc}`\n")
        print("[product_delivery_policy_guard] FAIL")
        print(f" - {exc}")
        return 1

    _write_json(OUT_JSON, report)
    _write(OUT_MD, "# Delivery Policy Guard\n\n- status: `PASS`\n")
    print("[product_delivery_policy_guard] PASS")
    return 0
