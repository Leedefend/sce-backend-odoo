...
const outDir = path.join(ARTIFACTS_DIR, 'codex', 'portal-shell-v0_9-4', ts);

function log(msg) {
  console.log(`[fe_workspace_tile_navigate_smoke] ${msg}`);
}
...
async function main() {
  ...
  const tileWithScene = tiles.find((tile) => tile.scene_key || tile.sceneKey || (tile.payload || {}).scene_key || (tile.payload || {}).sceneKey);
  const sceneKey = tileWithScene
    ? tileWithScene.scene_key || tileWithScene.sceneKey || (tileWithScene.payload || {}).scene_key || (tileWithScene.payload || {}).sceneKey
    : null;
  ...
  if (!sceneKey) {
    throw new Error('no tile with scene_key found');
  }
  if (!sceneKeys.includes(sceneKey)) {
    throw new Error(`scene_key not found in scenes: ${sceneKey}`);
  }

  log('PASS workspace tile navigation');
  log(`artifacts: ${outDir}`);
}

main().catch((err) => {
  console.error(`[fe_workspace_tile_navigate_smoke] FAIL: ${err.message}`);
  process.exit(1);
});
