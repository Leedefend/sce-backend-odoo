...
SNAPSHOT = ROOT / "artifacts" / "v2" / "execute_button_contract_snapshot_v1.json"
...
def run_audit() -> Dict[str, object]:
    ...
    if snapshot.get("snapshot_id") != "execute_button_contract_snapshot_v1":
    ...
    success = dispatch_intent(
        ...
        context={"user_id": 17, "company_id": 5, "trace_id": "exec-btn-contract-1637-pass"},
    )
    ...
    if str(meta.get("intent") or "") != "execute_button":
    ...
    failure = dispatch_intent(
        intent="execute_button",
        payload={"model": "res.partner", "record_id": 0, "button_name": "action_confirm"},
        context={"user_id": 17, "company_id": 5, "trace_id": "exec-btn-contract-1637-fail"},
    )
