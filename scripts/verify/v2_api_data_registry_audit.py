...
def run_audit() -> Dict[str, object]:
    ...
    from v2.intents.schemas.api_data_schema import ApiDataRequestSchemaV2  # type: ignore

    registry = build_default_registry()
    try:
        entry = registry.get("api.data")
    except KeyError:
        entry = None
        errors.append("api_data_not_registered")

    if entry is not None:
        if str(getattr(entry, "canonical_intent", "")) != "api.data":
        ...
        for expected_tag in ("api", "data", "read_write", "registry_closure"):
            if expected_tag not in tags:
                errors.append(f"missing_tag:{expected_tag}")

        if "ApiDataRequestSchemaV2" not in str(getattr(entry, "request_schema", "")):
            errors.append("request_schema_path_mismatch")
        if entry.handler_factory is not ApiDataHandlerV2:
            errors.append("handler_factory_mismatch")

    try:
        schema_result = ApiDataRequestSchemaV2.validate(
            {"model": "res.partner", "operation": "read"},
            {"trace_id": "api-data-reg-1638"},
        )
    ...
    if "api.data" not in migrated:
        errors.append("comparison_missing_migrated_api_data")
