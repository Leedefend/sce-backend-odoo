...
def run_audit() -> Dict[str, object]:
    ...
    result = dispatch_intent(
        ...
        context={"user_id": 7, "company_id": 2, "trace_id": "exec-btn-1635"},
    )
    ...
    if str(data.get("intent") or "") != "execute_button":
        errors.append("intent_mismatch")
    if str(data.get("model") or "") != "res.partner":
        errors.append("model_mismatch")
    if int(data.get("record_id") or 0) != 1:
        errors.append("record_id_mismatch")
    if str(data.get("button_name") or "") != "action_confirm":
        errors.append("button_name_mismatch")
