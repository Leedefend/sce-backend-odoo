...
SNAPSHOT = ROOT / "artifacts" / "v2" / "session_bootstrap_contract_snapshot_v1.json"
...
def run_audit() -> Dict[str, object]:
    ...
    if snapshot.get("snapshot_id") != "session_bootstrap_contract_snapshot_v1":
    ...
    success = dispatch_intent(
        ...
        context={"user_id": 17, "company_id": 5, "trace_id": "contract-1624-pass"},
    )
    ...
    if str(meta.get("intent") or "") != "session.bootstrap":
    ...
    failure = dispatch_intent(
        intent="session.bootstrap",
        payload={"app_key": 123},
        context={"user_id": 17, "company_id": 5, "trace_id": "contract-1624-fail"},
    )
