...
OUT_JSON = ROOT / "artifacts" / "backend" / "product_project_plan_block_contract_guard.json"

RESPONSE_KEYS = {"project_id", "block_key", "block", "degraded"}
SUPPORTED = {
    "plan_summary_detail": "plan_summary_detail",
    "plan_tasks": "plan_task_list",
...
def main() -> int:
    ...
    try:
        ...
        status, create_resp = _post(intent_url, token, "project.initiation.enter", {
            "name": f"P13B-PLAN-BLOCK-{uuid4().hex[:8]}",
            "description": "plan block contract guard",
        ...
        for block_key, expected_type in SUPPORTED.items():
            status, block_resp = _post(intent_url, token, "project.plan_bootstrap.block.fetch", {"project_id": project_id, "block_key": block_key}, db_name=db_name)
            _assert_ok(status, block_resp, f"project.plan_bootstrap.block.fetch({block_key})")
        ...
        print("[product_project_plan_block_contract_guard] FAIL")
        print(f" - {exc}")
        return 1

    _write_json(OUT_JSON, report)
    print("[product_project_plan_block_contract_guard] PASS")
    return 0
