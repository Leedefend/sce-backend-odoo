...
SNAPSHOT = ROOT / "artifacts" / "v2" / "file_upload_contract_snapshot_v1.json"
...
def run_audit() -> Dict[str, object]:
    ...
    if snapshot.get("snapshot_id") != "file_upload_contract_snapshot_v1":
    ...
    success = dispatch_intent(
        ...
        context={"user_id": 17, "company_id": 5, "trace_id": "file-upload-contract-1661-pass"},
    )
    ...
    if str(meta.get("intent") or "") != "file.upload":
    ...
    failure = dispatch_intent(
        intent="file.upload",
        payload={"model": "", "res_id": 1, "data": "dGVzdA=="},
        context={"user_id": 17, "company_id": 5, "trace_id": "file-upload-contract-1661-fail"},
    )
