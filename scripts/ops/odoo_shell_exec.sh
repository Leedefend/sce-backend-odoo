...
while IFS='=' read -r env_name _; do
  case "$env_name" in
    APPLY|ARCHIVE_*|MIGRATION_*|DIRECT_ACCEPTANCE_*|FRESH_DB_*|LEGACY_USER_*|LEGACY_ATTACHMENT_*|PROJECT_ANCHOR_*|PROJECT_MASTER_*|BUSINESS_FACT_*|BUSINESS_CONFIG_*|PRODUCT_*|CONTRACT_*|CONSTRUCTION_CONTRACT_*|SUPPLIER_CONTRACT_*|LOWCODE_*|SC_RUNTIME_*|SCBS55_*|ROLE_*|SC_ENVIRONMENT|SC_ALLOW_DEMO_DATA|PARTNER_ASSET_XML|PARTNER_BUSINESS_ALIGNED_GATE_CSV|PARTNER_FACT_ALIGNMENT_*|PARTNER_PROFILE_BACKFILL_*|PARTNER_SOURCE_CREATOR_*)
      if [[ -n "${!env_name:-}" ]]; then
