...
export function useActionViewHudEntriesRuntime(options: UseActionViewHudEntriesRuntimeOptions) {
  function buildHudEntries(): HudEntry[] {
    ...
    return [
      ...
      { label: '契约视图', value: input.contractViewType || '-' },
      { label: '契约筛选', value: input.activeContractFilterKey || '-' },
      ...
      { label: '契约操作数', value: input.contractActionCount || 0 },
      { label: '契约分页数', value: input.contractLimit || 0 },
      { label: '允许读取', value: input.contractReadAllowed },
      { label: '契约提醒数', value: input.contractWarningCount || 0 },
